...
//		echo $sel_val;
	...
	function	 REMARKS_LOOKUP($conn)
	{
		...
		$retval	 =	"<select id='remarks_' name='remarks_' style='font-size:12px;' >";
...
<body>
	<form name="dataform" id="dataform" enctype="multipart/form-data" method="POST">
		...
		<div id="divdata_add" title="INVENTORY ADJUSTMENT">
			<table width="100%" border="1" class="Text_header" style="border:0px">
				<tr>
					<td align="left" width="20%" style="font-size:15px">
						I.A. DATE
					</td>
					<td align="left" width="80%" style="font-size:15px">
						:<input type="hidden" name="hdniadate" id="hdniadate" value="" maxlength="10" size="10">
					</td>
				</tr>
				<tr>
					<td align="left" width="20%" style="font-size:15px">
					...
					<td align="left" width="80%" style="font-size:15px">
						:<input type="text" name="txtcustname" id="txtcustname" value="" maxlength="10" size="10">
					</td>
				</tr>
				<tr>
					<td align="left" width="20%" style="font-size:15px">
					...
					<td align="left" width="80%" style="font-size:15px">
						...
						<input type="radio" name="rdreftype" id="rdinvoice">INVOICE
						<input type="radio" name="rdreftype" id="rdstf">STF
						<input type="radio" name="rdreftype" id="rdprr">PRR
					</td>
				</tr>
				<tr>
					<td align="left" width="20%" style="font-size:15px">
				...
				<tr>
					...
					<td align="left" width="80%" style="font-size:15px">
						:<input type="text" name="textitemno" id="textitemno" value="" maxlength="10" size="10" onkeyup="if(event.keyCode==13) { Get_item(); } ">
		...
		<div id="div_item" style="display:none;" align="center" ></div>
...
function DISPLAY()
{
	...
	$.ajax({
			data		:	dataform,
			type		:	'POST',
			url			:	'adjustment_test.php?action=DISPLAY_&DFROM='+dfrom+'&DTO='+dto,
			beforeSend	:	function()
						{
...
function Get_item()
{
	...
	{
		$.ajax({
				url			:	'adjustment_test.php?action=GETITEM&ITEMNO='+Itemno,
				beforeSend	:	function()
							{
								$('#divloader').dialog('open');
							},
...
function searchcust(evt)
{
	...
	if(custcode != '' || custname != '')
	{
		...
		{
			$.ajax({
				url			:	'adjustment_test.php?action=SEARCHCUST&CUSTCODE='+custcode+'&CUSTNAME='+custname,
				success		:	function(response)
				{
					if(response == 'zero')
					{
...
{
	$.ajax({
			url			:	'adjustment_test.php?action=ITEM_LIST&VAL_IANO='+val_iano,
			beforeSend	:	function()
						{
							$('#divloader').dialog('open');
						},
...
$("#divdata_detail").dialog({
	...
	buttons: {
		...
		'SUBMIT': function()
		{
			...
			{
				$.ajax({
						url			:	'adjustment_test.php?action=POSTING&THIS_IANO='+this_iano,
						beforeSend	:	function()
									{
										$('#divloader').dialog('open');
									},
...
$("#divdata_add").dialog({
	dialogClass: "no-close",
	bgiframe:true, resizable:false, height: 500, width: 900, modal:true, autoOpen: true,	draggable: false,
	buttons: {
		'CLOSE': function()
		{
			$(this).dialog('close');
