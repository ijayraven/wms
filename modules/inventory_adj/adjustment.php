...
if ($rsQuery == false) 
{
	...
	if ($action=='ITEM_LIST') 
	{
		...
		$show	.=	"<form id='frmtrxdtls'><table width='100%' border='0' id='tbltrxdtls'>";
		$show	.=	"<tr style='font-size:15px;'>";
		$show	.=		"<td width='5%' nowrap colspan='7' align='center' id='tdtrxno'>";
		$show	.=			"<b>".$_GET['VAL_IANO'];
		$show	.=		"</td>";
		$show	.=	"</tr>";
		$show	.=	"<tr class='Header_style' style='font-size:15px;'>";
		$show	.=		"<td width='5%' nowrap>";
		...
		while (!$rsIANO->EOF) 
		{
			...
			$show	.=	"<td align='center' id='tdskuno$COUNTER' >";
			$show	.=	$SKUNO;
			$show	.=	"</td>";
			...
			$show	.=	"<td align='center'   nowrap id = 'tdrem$COUNTER'>";
			$show	.=	$REMARKS;
			$show	.=	"</td>";
		...
		if (substr($_GET['VAL_IANO'],0,3) == 'IAM')
		{
			$show	.=	"<input type='button' value='Add Row' id='btnaddrow' onclick='addrow();'>";
		}
		$show	.=	"<input type='hidden' id='txtrownum' name='txtrownum'>";
		$show	.=	"<input type='hidden' id='txtrowcnt' name='txtrowcnt' value='$COUNTER'>";
		$show	.=	"</form>";
		echo $show;
		exit();
	}
	...
	if ($action=='GETITEMDTLS') 
	{
		$ITEMNO	=	$_GET["ITEMNO"];
		$cnt	=	$_GET["CNT"];
		$itemdesc	=	$global_func->Select_val($Filstar_conn,"FDCRMSlive","itemmaster","ItemDesc","ItemNo = '$ITEMNO'");
		$itemstatus	=	$global_func->Select_val($Filstar_conn,"FDCRMSlive","itemmaster","DeptNo","ItemNo = '$ITEMNO'");
		$itemcat	=	$global_func->Select_val($Filstar_conn,"FDCRMSlive","itemmaster","ItemCategory","ItemNo = '$ITEMNO'");
		$whcode		=	$global_func->Select_val($Filstar_conn,"FDCRMSlive","itembal","whsloc","itmnbr = '$ITEMNO' AND house='FDC'");
		if($itemstatus == "M")
		{
			if($itemcat == "4")
			{
				...
				{
					echo "<script>
							$('#tditemdesc$cnt').text('$itemdesc');
							$('#tdlocation$cnt').text('$whcode');
						  </script>";
				}
				else 
				{
					echo "<script>
							alert('Item is not in itembal.');
							$('#txtsku$cnt').val('');
							$('#tditemdesc$cnt').text('');
							$('#tdlocation$cnt').text('');
						  </script>";
			...
			{
				echo "<script>
						alert('Item is not a finished good.');
						$('#txtsku$cnt').val('');
						$('#tditemdesc$cnt').text('');
						$('#tdlocation$cnt').text('');
					  </script>";
	...
	if($action == "SAVEITEM")
	{
		...
		$itemstatus	=	$global_func->Select_val($Filstar_conn,"FDCRMSlive","itemmaster","DeptNo","ItemNo = '$ITEMNO'");
//		$whcode		=	$global_func->Select_val($Filstar_conn,"FDCRMSlive","itembal","whsloc","itmnbr = '$ITEMNO' AND house='FDC'");
		$REMARKS	=	$global_func->Select_val($Filstar_conn,"WMS_NEW","INVENTORYADJUSTMENT_DTL","REMARKS"," IATRANSNO='$TRXNO'");
		
		$inseritem	=	"INSERT INTO WMS_NEW.INVENTORYADJUSTMENT_DTL(`IATRANSNO`, `SKUNO`, `ITEMSTATUS`, `HOUSE`, `IA_TYPE`, `MOVEMENT`, `IAQTY`, `REMARKS`)
						 VALUES('$TRXNO','$ITEMNO','$itemstatus','FDC','ONHAND','$MOVEMENT','$QTY','$REMARKS')";
		$rsinseritem	=	$Filstar_conn->Execute($inseritem);
		if ($rsinseritem==false) 
		{
			echo $Filstar_conn->ErrorMsg()."::".__LINE__;exit();
		}
		else 
	...
	if($action == "DELITEM")
	{
		...
		$DELETEITEM		=	"DELETE FROM WMS_NEW.INVENTORYADJUSTMENT_DTL WHERE IATRANSNO = '$TRXNO' AND SKUNO = '$ITEMNO'";
		$RSDELETEITEM	=	$Filstar_conn->Execute($DELETEITEM);
		if($RSDELETEITEM == false)
		{
			echo $Filstar_conn->ErrorMsg()."::".__LINE__; exit();
		}
		else 
...
<body onload="truncate();">
		...
		<div id="divdata_add_hdr" style="display:none;">
			<table width="100%" border="0" class="Text_header" style="border:0px;">
				...
				<tr>
					...
					<td align="left" width="80%" style="font-size:15px">
						:&nbsp;<input type="text" name="textrefno" id="textrefno" value="" maxlength="14" size="14">
	...
	<div id="divmovement" class="Text_header" align="center">
		MOVEMENT	: <select id='selmovement' name="selmovement">
...
$("#divmovement").dialog({
	...
	buttons: {
		...
		{
			var a			=	$(this).data("CNT");
			var txtitemno 	= 	$("#txtsku"+a).val();
			var	txtqty	 	=	$("#txtqty"+a).val();
			var	selmovement	=	$("#selmovement").val();
			var	tdtrxno	 	=	$("#tdtrxno").text();
			if(selmovement != "")
			{
				$.ajax({
					url			:	'adjustment.php?action=SAVEITEM&ITEMNO='+txtitemno+"&QTY="+txtqty+"&TRXNO="+tdtrxno+"&MOVEMENT="+selmovement,
					beforeSend	:	function()
								{
									$('#divloader').dialog('open');
								},
					success	:	function(response)
								{
									...
									$('#txtsku'+a).attr('readonly', true);
									$('#txtqty'+a).attr('readonly', true);
									$('#divmovement').dialog('close');
									$("#selmovement").val("");
									$('#divloader').dialog('close');
								}
				});
...
function addrow(){
	var row	=	"";
	var txtrownum	=	+ $("#txtrownum").val();
	var txtrowcnt	=	+ $("#txtrowcnt").val();
	var remarks		=	$("#tdrem1").text();
	txtrownum++;
	row = "<tr onMouseOver=\"bgColor='#00FFFF';\" onmouseout=\"bgColor='#cccccc';\" bgcolor=\"#cccccc\" class='Text_header_hover' style='font-size:13px;' id='trtrxdtl"+txtrownum+"' >";
	row += 	"<td align='center'>";
	row += 	txtrowcnt;
	row += 	"</td>";
	row += 	"<td>";
	row += 	"<input type='text' id='txtsku"+txtrownum+"' name='txtsku"+txtrownum+"' size='10' placeholder='Item No.' style='text-align:center;' onchange='getitem(this.value,"+txtrownum+");saveitem("+txtrownum+");'>";
	row += 	"</td>";
	row += 	"<td align='center' id='tditemdesc"+txtrownum+"'>";
	row += 	"";
	row += 	"</td>";
	row += 	"<td align='center'>";
	row += 	"<input type='text' id='txtqty"+txtrownum+"' name='txtqty"+txtrownum+"' size='5' placeholder='Qty'onkeyup='isnumeric_(this.value,this.id);' style='text-align:center;' onchange='saveitem("+txtrownum+");'>";
	row += 	"</td>";
	row += 	"<td align='center' id='tdremarks"+txtrownum+"'>";
	row += 	remarks;
	row += 	"</td>";
	row += 	"<td align='center' id='tdlocation"+txtrownum+"'>";
	row += 	"";
	row += 	"</td>";
	row += 	"<td align='center'>";
//	row += 	"<img src='../../images/action_icon/edit-icon.gif' onclick=EDIT_this('','','{$IAQTY}'); title='EDIT'; style='width:17px;'>&nbsp;&nbsp;";	
	row += 	"<img src='../../images/action_icon/delete-icon.gif' onclick=remrow("+txtrownum+"); title='DELETE ROW'; style='width:17px;'>";
	row += 	"</td>";
	row += "</tr>";
	
	txtrowcnt++;
	
	$('#tbltrxdtls tr:last').after(row);
	$("#txtrowcnt").val(txtrowcnt);
	$("#txtrownum").val(txtrownum);
}
function getitem(itemno,cnt)
{
	if(itemno != "")
	{
		var founddup = vchkdup(itemno,cnt);
		if(!founddup)
		{
			$.ajax({
				url			:	'adjustment.php?action=GETITEMDTLS&ITEMNO='+itemno+"&CNT="+cnt,
				beforeSend	:	function()
							{
								$('#divloader').dialog('open');
							},
...
function vchkdup(itemno,cnt)
{
	...
	var tditems;
	var a			=	0;
	for(a=1; a <= txtrowcnt; a++)
	{
		if(a != cnt)
		{
			tditems = $("#tdskuno"+a).text();
	...
	for(a=1; a <= txtrownum; a++)
	{
		if(a != cnt)
		{
			txtitems = $("#txtsku"+a).val();
...
function remrow(cnt)
{
	if(confirm("You are about to delete this item from this transaction, please confirm."))
	{
		var	tdtrxno	 	=	$("#tdtrxno").text();
		var itemno		=	$("#txtsku"+cnt).val();
		
		$.ajax({
			url			:	'adjustment.php?action=DELITEM&ITEMNO='+itemno+"&TRXNO="+tdtrxno,
			beforeSend	:	function()
						{
							$('#divloader').dialog('open');
						},
...
function validate()
{
	...
	for(var a = 1; a <= txtrownum; a++)
	{
		...
		{
			notempty = false;
			$("#trtrxdtl"+a).css("background-color","#ff6666");
		}
		else
		{
			$("#trtrxdtl"+a).css("background-color","#cccccc");
		}
	}
	return notempty;
}
function saveitem(a)
{
	...
	if(txtitemno != "" && txtqty != "")
	{
		$('#divmovement').data('CNT', a)
		$("#divmovement").dialog("open");
