...
class PDF extends FPDF 
{
	...
	function Footer()
	{
		...
		$this->SetY(274);$this->Cell(0,10,'Reprinted Date  : '.date('Y-m-d H:i:s'),0,1,'L');
...
foreach ($arrMPOS as $MPOSNO=>$val1)
{
	...
	$Filstar_conn->StartTrans();
		$TRANS_SCANNING	=	"UPDATE WMS_NEW.SCANDATA_HDR SET `REPRINTDT`=NOW(),REPRINTBY='{$_SESSION['username']}'
							 WHERE MPOSNO = '{$MPOSNO}'";
	...
	$pdf->SetX(160);$pdf->Cell(40,5,number_format($totSCANNEDAMT,2),1,1,'R');
	$pdf->SetX(10);$pdf->Cell(190,5,"****REPRINT****",1,0,'C');
	$pdf->ln(10);
