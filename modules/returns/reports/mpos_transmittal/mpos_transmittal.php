...
if($action == "SEARCHMPOS")
{
	...
	else 
	{
		...
		if($RSGETNONREV == false)
		{
			...
			$DATASOURCE->logError("wms",$errmsg,$GETNONREV,$_SESSION['username'],"MPOS TRANSMITTAL","SEARCHMPOS");
	...
	{
		$errmsg	=	($conn_255_10->ErrorMsg()."::".__LINE__); 
		$DATASOURCE->logError("wms",$errmsg,$GETSCANNEDMPOS,$_SESSION['username'],"MPOS TRANSMITTAL","SEARCHMPOS");
		$DATASOURCE->displayError();
	}
	else 
	{
...
$("document").ready(function(){
	$("#btnsearch").click(function(){
		...
		else
		{
			if(selcustype == "")
			{
				errmsg	+=	"Please select Customer Type.";
			}
			if(posteddfrom == "" && posteddto == "")
			{
				errmsg	+=	"Specify date range.";
			}
		...
		if(errmsg == "")
		{
			$.ajax({
				data		:dataform,
				type		:"POST",
				url			:"mpos_transmittal.php?action=SEARCHMPOS",
				beforeSend	:function(){
	...
	$("#btngenerate").click(function(){
		...
		if(errmsg == "")
		{
			window.open("mpos_transmittal_pdf.php?CUSTYPE="+selcustype+"&POSTEDDFROM="+posteddfrom+"&POSTEDDTO="+posteddto);
		}
		else
		{
			MessageType.infoMsg(errmsg);
