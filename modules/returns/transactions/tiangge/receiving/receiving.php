...
if($action == "GETMTO")
{
	...
	$GETMTO		=	"SELECT `MTONO`,`STATUS`, `MTO_TRANSMITTED_DT`,`RECEIVEDBY`, `RECEIVEDDATE` FROM WMS_NEW.MTO_TIANGGE_HDR 
...
if($action == "RECEIVEMTO")
{
	if(!empty($_POST['chkpick']))
	{
		...
		$RECEIVEMTO		=	"UPDATE WMS_NEW.MTO_TIANGGE_HDR SET STATUS = 'RECEIVED', RECEIVEDBY = '{$_SESSION['username']}',RECEIVEDDATE='$today',RECEIVEDTIME = '$TIME'
						 	 WHERE MTONO IN ($MTOlist)";
		$RSRECEIVEMTO	=	$DATASOURCE->execQUERY("wms",$conn_255_10,$RECEIVEMTO,$_SESSION["username"],"MTO RECEIVING","RECEIVEMTO");
		if($RSRECEIVEMTO)
		{
			echo "<script>
...
include($_SERVER['DOCUMENT_ROOT']."/wms/modules/returns/transactions/tiangge/receiving/receiving.html");
?>