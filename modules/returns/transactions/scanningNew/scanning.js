...
//					$("#divloader").dialog("close");
		...
		saveScannedItems:function(mode)
		{
			var	MPOSNO	=	$("#tdmposno").text();
			$.ajax({
				url		:	'scanning.php?action=SAVESCANNEDITEMS&MPOSNO='+MPOSNO+"&mode="+mode,
				beforeSend:	function()
				{
					$("#divloader").dialog("open");
				},
		...
		truncateTmpTbl:function()
		{
			$.ajax({
				url			:	'scanning.php?action=TRUNCATE',
				beforeSend	:	function()
							{
							},
				success		:	function(response)
...
$("document").ready(function(){
    ...
    $("#divMPOS").on("click",".scanitems",function(){
		var MPOSNO	=	$(this).attr("data-mposno");
		$('.diaMPOS .ui-button-text:contains(Update)').text('Save');
		MessageType.confirmmsg(scanningFuns.save_scan,"You are about to save this MPOS and proceed to item/s scanning. Do you want to continue?",MPOSNO);
	});
    $("#divMPOS").on("click",".editdtls",function(){
		var MPOSNO	=	$(this).attr("data-mposno");
		$('.diaMPOS .ui-button-text:contains(Save)').text('Update');
