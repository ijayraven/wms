...
if($action == "CREATEEXMTO")
{
	...
	else
	{
		...
		while (!$RSGETEXITEMS->EOF)
		{
			...
			$SCANNEDQTY		=	$global_func->Select_val($Filstar_conn,"WMS_NEW","SCANDATA_DTL AS D LEFT JOIN WMS_NEW.SCANDATA_HDR AS H ON H.MPOSNO = D.MPOSNO","SUM(POSTEDQTY)","SKUNO= '{$ITEMNO}' AND STATUS = 'POSTED'");
			$DEFECTIVEQTY	=	$global_func->Select_val($Filstar_conn,"WMS_NEW","SCANDATA_DTL AS D LEFT JOIN WMS_NEW.SCANDATA_HDR AS H ON H.MPOSNO = D.MPOSNO","SUM(DEFECTIVEQTY)","SKUNO= '{$ITEMNO}' AND STATUS = 'POSTED'");
			...
			$POSTEDMTOQTY	=	$global_func->Select_val($Filstar_conn,"WMS_NEW","MTO_RTN_EXCDTL AS D LEFT JOIN WMS_NEW.MTO_RTN_EXCHDR AS H ON H.MTONO = D.MTONO","SUM(D.QTY)","D.SKUNO= '{$ITEMNO}' AND (H.STATUS != 'CANCELLED')");
			$ITEMSTATUS		=	$global_func->Select_val($Filstar_pms,"FDC_PMS","ITEMMASTER","ITEMSTATUS","ITEMNO = '$ITEMNO'");
			$ONHAND			=	$SCANNEDQTY - $DEFECTIVEQTY - $POSTEDMTOQTY - $POSTEDRTNMTOQTY;
			if($ITEMSTATUS != "P")
			{
				if($ONHAND > 0)
				{
...
//			$POSTEDDT 		= 	$global_func->Select_val($Filstar_conn,"WMS_NEW","MTO_RTN_EXCDTL","SUM(QTY)","SKUNO= '{$SKUNO}' AND MTONO = '{$TRXNO}'");
			
			$SCANNEDQTY		=	$global_func->Select_val($Filstar_conn,"WMS_NEW","SCANDATA_DTL AS D LEFT JOIN WMS_NEW.SCANDATA_HDR AS H ON H.MPOSNO = D.MPOSNO","SUM(POSTEDQTY)","SKUNO= '{$ITEMNO}' AND STATUS = 'POSTED' AND H.POSTEDDATE >= '$POSTEDDT'");
			$DEFECTIVEQTY	=	$global_func->Select_val($Filstar_conn,"WMS_NEW","SCANDATA_DTL AS D LEFT JOIN WMS_NEW.SCANDATA_HDR AS H ON H.MPOSNO = D.MPOSNO","SUM(DEFECTIVEQTY)","SKUNO= '{$ITEMNO}' AND STATUS = 'POSTED' AND H.POSTEDDATE >= '$POSTEDDT'");
			$POSTEDRTNMTOQTY=	$global_func->Select_val($Filstar_conn,"WMS_NEW","MTO_RTNDTL AS D LEFT JOIN WMS_NEW.MTO_RTNHDR AS H ON H.MTONO = D.MTONO","SUM(QTY)","SKUNO= '{$ITEMNO}' AND H.POSTDATE != '0000-00-00'");
			$CURMTOQTY		=	$global_func->Select_val($Filstar_conn,"WMS_NEW","MTO_RTN_EXCDTL","SUM(QTY)","SKUNO= '{$SKUNO}' AND MTONO = '{$TRXNO}'");
			$MTOQTY			=	$global_func->Select_val($Filstar_conn,"WMS_NEW","MTO_RTN_EXCDTL AS D LEFT JOIN WMS_NEW.MTO_RTN_EXCHDR AS H ON H.MTONO = D.MTONO","SUM(D.QTY)","D.SKUNO= '{$SKUNO}' AND (H.STATUS != 'CANCELLED')");
//			$ONHAND			=	$CURMTOQTY + $SCANNEDQTY - $DEFECTIVEQTY-$MTOQTY + $POSTEDRTNMTOQTY;
			$table	.=		"<tr id='tr$cnt' class='trbody'>
								<td id='tdcurcnt$cnt' align='center'>$cnt</td>
