...
$PRINTMTO	=	"UPDATE WMS_NEW.MTO_PRIMESTOCK_HDR SET `STATUS` = 'PRINTED',`PRINTBY`='{$_SESSION['username']}', `PRINTDATE`='{$TODAY}', `PRINTTIME`='{$time}'
				 WHERE `MTONO` = '{$TRXNO}'";
							...
							foreach ($val4 as $SRP_A=>$val5)
							{
								...
								foreach ($val5 as $LOC_A=>$val6)
								{
									...
									foreach ($val6 as $SKUNO_A=>$val7)
									{
										...
										$pdf->SetX(180);$pdf->Cell(30,5,$LOC_A,0,0,'C');
										$pdf->SetX(210);$pdf->Cell(30,5,number_format($SRP_A,2),0,0,'C');
