...
if($action == "CREATEEXMTO")
{
	...
	$txtsto		=	$_GET["txtsto"];
	$GETPRIMEITEMS	=	"SELECT D.MPOSNO, D.`SKUNO`, D.`ITEMSTATUS`, SUM((D.`POSTEDQTY` - D.`DEFECTIVEQTY` - D.`IB_QTY`)) AS QTY, SUM(D.`POSTEDQTY`),SUM(D.`IB_QTY`), SUM(D.`DEFECTIVEQTY`), D.`DELBY`,D.MTOPRIMECREATED FROM WMS_NEW.SCANDATA_DTL AS D
					 ...
					 GROUP BY D.`SKUNO`";
	$RSGETPRIMEITEMS	=	$conn_255_10->Execute($GETPRIMEITEMS);
	...
	{
		if($RSGETPRIMEITEMS->RecordCount() == 0)
		{
			echo "<script>
		...
		else 
		{
			...
			while (!$RSGETPRIMEITEMS->EOF)
			{
				...
				$ITEMNO 		= 	$RSGETPRIMEITEMS->fields["SKUNO"]; 
				$ITEMSTATUS		= 	$RSGETPRIMEITEMS->fields["ITEMSTATUS"];
				$POSTEDQTY		= 	$RSGETPRIMEITEMS->fields["QTY"];
				$ITEMDESC		=	$DATASOURCE->selval($conn_250_171,"FDC_PMS","ITEMMASTER","ITEM_DESC","ITEMNO = $ITEMNO");
				$DEFECTIVEQTY	=	$RSGETPRIMEITEMS->fields["DEFECTIVEQTY"];
				$ONHAND			=	$POSTEDQTY;
				if($ONHAND > 0)
				{
		...
		else
		{
			while (!$RSGETPRIMEITEMS->EOF)
			{
				...
				if($ONHAND > 0)
				{
					...
					$GROSSAMOUNT= $ONHAND * $UNITPRICE;
					$SAVEMTODTLS	=	"INSERT INTO WMS_NEW.MTO_PRIMESTOCK_DTL(`MTONO`,`SKUNO`, `DESCRIPTION`, `ITEMSTATUS`, `QTY`,`NO_OF_BOXES`, `NO_OF_PACK`, `UNITPRICE`, `GROSSAMT`, `RANGESCANFROM`, `RANGESCANTO`)
					...
					$UPDATEDTL 	=	"UPDATE WMS_NEW.SCANDATA_DTL AS D
									 ...
									 WHERE H.SCANDATE BETWEEN '$txtsfrom' AND '$txtsto' AND D.SKUNO = '$ITEMNO'";
