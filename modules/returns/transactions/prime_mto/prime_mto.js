$("document").ready(function(){
	$("#btnsearchitems").click(function(){
		...
		{
			$.ajax({
				url			:	"prime_mto.php?action=CREATEEXMTO&txtsfrom="+txtsfrom+"&txtsto="+txtsto,
				beforeSend	:function(){
					$("#divloader").dialog("open");
				},
				success		:function(response){
					$("#divtrxmto").html(response);
					$('#divtrxmto').dialog('option', 'title', 'PRIME RETURNS MTO CREATION');
...
//		var destination	=	$('input[name=rdodestination]:checked', '#dataform').val();
		...
		if(valid)
		{
			$.ajax({
				data		:dataform,
				type		:"POST",
				url			:"prime_mto.php?action=GETMTO&USESESSIONQUERY="+usequery,
				beforeSend	:function(){
					$("#divloader").dialog("open");
				},
				success		:function(response){
   ...
   $("#divrtnmto").on("click",".editbtn",function(){
    	var TRXNO	=	$(this).attr("data-trxno");
    	 $.ajax({
				type	:	"GET",
				url		:	"prime_mto.php?action=EDITTRX&TRXNO="+TRXNO,
				beforeSend:	function()
				{
					$("#divloader").dialog("open");
				},
				success	:function(response)
				{
			    	...
			    	$('#divtrxmto').dialog('option', 'title', 'PRIME RETURNS MTO UPDATE');
     ...
     $("#divrtnmto").on("click",".postbtn",function(){
    	...
    	if(confirm("You are about to post this transaction."))
    	{
	    	 $.ajax({
					type	:	"GET",
					url		:	"prime_mto.php?action=POSTTRX&TRXNO="+TRXNO,
					beforeSend:	function()
					{
						$("#divloader").dialog("open");
					},
     ...
     $("#divrtnmto").on("click",".cancelbtn",function(){
    	...
    	if(confirm("You are about to cancel this transaction."))
    	{
	    	 $.ajax({
					type	:	"GET",
					url		:	"prime_mto.php?action=CANCELTRX&TRXNO="+TRXNO,
					beforeSend:	function()
					{
						$("#divloader").dialog("open");
					},
    ...
    $("#divrtnmto").on("click",".printbtn",function(){
    	var TRXNO	=	$(this).attr("data-trxno");
    	window.open("prime_mto_pdf.php?TRXNO="+TRXNO);
    	window.open("prime_mto__summary_pdf.php?TRXNO="+TRXNO);
    });
    $("#divrtnmto").on("click",".tdtrxdtls",function(){
		...
		{	
		    $.ajax({
				type	:	"GET",
				url		:	"prime_mto.php?action=VIEWTRXDTLS&TRXNO="+TRXNO+"&COUNT="+COUNT,
				beforeSend:	function()
				{
					$("#divloader").dialog("open");
				},
    ...
    $("#divrtnmto").on("click",".transmitbtn",function(){
    	...
    	if(confirm("You are about to transmit this MTO."))
    	{
	    	 $.ajax({
					type	:	"GET",
					url		:	"prime_mto.php?action=TRANSMITTRX&TRXNO="+TRXNO,
					beforeSend:	function()
					{
						$("#divloader").dialog("open");
					},
...
//		var destination	=	$('input[name=rdodestination_C]:checked', '#dataform').val();
		...
		$.ajax({
				data		:frmdata,
				type		:"POST",
				url			:"prime_mto.php?action=SAVETRXHDR&txtsfrom="+txtsfrom+"&txtsto="+txtsto+"&txtnoboxes="+txtnoboxes+"&txtnopackages="+txtnopackages,
				beforeSend	:function(){
					$("#divloader").dialog("open");
				},
				success		:function(response)
...
function updatetrx()
{
	...
	$.ajax({
			data		:frmdata,
			type		:"POST",
			url			:"prime_mto.php?action=UPDATETRX&TRXNO="+TRXNO,
			beforeSend	:function(){
				$("#divloader").dialog("open");
			},
			success		:function(response){
