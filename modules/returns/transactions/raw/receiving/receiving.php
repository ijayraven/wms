...
if($action == "GETMTO")
{
	...
	$GETMTO		=	"SELECT `MTONO`,`STATUS`, `MTO_TRANSMITTED_DT`,`RECEIVEDBY`, `RECEIVEDDATE` FROM WMS_NEW.MTO_RAWHDR 
...
if($action == "RECEIVEMTO")
{
	if(!empty($_POST['chkpick']))
	{
		...
		$RECEIVEMTO		=	"UPDATE WMS_NEW.MTO_RAWHDR SET STATUS = 'RECEIVED', RECEIVEDBY = '{$_SESSION['username']}',RECEIVEDDATE='$today',RECEIVEDTIME = '$TIME'
...
include($_SERVER['DOCUMENT_ROOT']."/wms/modules/returns/transactions/raw/receiving/receiving.html");
?>