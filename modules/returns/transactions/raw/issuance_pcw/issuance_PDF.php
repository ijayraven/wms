...
class PDF extends FPDF 
{
	function Header()
	{
		...
		$PCWORKER	= 	$global_func->Select_val($Filstar_conn,"WMS_NEW","MTO_RAWHDR","PCWORKER","MTONO= '{$TRXNO}'");
...
$GETMTO	=	"SELECT * FROM WMS_NEW.MTO_RAWHDR WHERE MTONO = '{$MTONO}'";
...
else 
{
	...
	while (!$RSGETMTO->EOF) {
		...
		$TOTALQTY	= $global_func->Select_val($Filstar_conn,"WMS_NEW","MTO_RAWDTL","SUM(QTY)","MTONO= '{$MTONO}'");
		$TOTALGROSS	= $global_func->Select_val($Filstar_conn,"WMS_NEW","MTO_RAWDTL","SUM(GROSSAMT)","MTONO= '{$MTONO}'");
