	      	...
	      	click: function() {
				{
					...
					if(confirm("You are about to transmit this MTO."))
			    	{
				    	 $.ajax({
								type	:	"GET",
								url		:	"ib_mto.php?action=TRANSMITTRX&TRXNO="+TRXNO+"&PCW="+pcw_add+"&ARS="+ars_add,
								beforeSend:	function()
								{
...
$("#divtrxmto").dialog({
	...
	closeOnEscape:false,	
	title:'INTERNAL BARCODE RETURN MTO CREATION',
	...
	[
		...
		
		{
			...
			click: function() 
			{
				...
				if(IB_MTO_funcs.validateItems())
				{
					MessageType.confirmmsg(IB_MTO_funcs.saveTrx,"Do you want to "+endmsg+" this transaction?",mode);
				}
			}
		}	
	]
...
$("#divsearchitems").dialog({
	dialogClass: "no-close",
	closeOnEscape:false,	
	title:'SEARCH INTERNAL BARCODE ITEMS',
