...
if($action == "CREATEIBMTO")
{
	...
	$GETIBITEMS	=	"SELECT D.MPOSNO, D.`SKUNO`, D.`ITEMSTATUS`, SUM(D.`IB_QTY`) AS QTY, D.`DELBY`,D.MTOIBCREATED FROM WMS_NEW.SCANDATA_DTL AS D
					 ...
					 WHERE H.POSTEDBY != '' AND D.`DELBY` = '' AND H.POSTEDDATE BETWEEN '$txtsfrom' AND '$txtsto' AND D.`IB_QTY` != 0
					 GROUP BY D.`SKUNO`";
	...
	else
	{
		...
		else 
		{
			...
			while (!$RSGETIBITEMS->EOF)
			{
				...
				$EXMTOITEM		= 	$DATASOURCE->selval($conn_255_10,"WMS_LOOKUP","MTO_EX_ITEMS_DTLS","ITEMNO","ITEMNO= '{$ITEMNO}' AND CANCELLED = 'N'");
				if($EXMTOITEM != "")
				{
					$TRX_NO		= $DATASOURCE->selval($conn_255_10,"WMS_LOOKUP","MTO_EX_ITEMS_DTLS","TRX_NO","ITEMNO= '{$ITEMNO}'  AND CANCELLED = 'N'");
					$STATUS		= $DATASOURCE->selval($conn_255_10,"WMS_LOOKUP","MTO_EX_ITEMS_HDR","STATUS","TRX_NO= '{$TRX_NO}'");
					if($STATUS == "POSTED")
					{
						$NONMTOITEM = true;
					}
				...
				if($ONHAND > 0 and $NONMTOITEM == false)
				{
					$table	.=		"<tr id='tr$cnt' class='trbody'>
										...
										<td align='center'>
											<input type='text' id='txtqty$cnt' name='txtqty$cnt' size='5' class='centered numbersonly txtqty' data-curcnt = '$cnt' value='$ONHAND'>
										</td>
										<td align='center'>
											$ITEMSTATUS
										</td>
...
{
	$hidcnt		=	$_POST["hidcnt"];
	$txtboxes	=	$_POST["txtboxes"];
	$txtpackages=	$_POST["txtpackages"];
	$txtsfrom	=	$_GET["txtsfrom"];
	$txtsto		=	$_GET["txtsto"];
	$mode		=	$_GET["mode"];
	...
	if($RSSAVEMTOTRX)
	{
		for ($a = 1; $a < $hidcnt; $a++)
		{
			...
			if($txtitemno)
			{
				if($mode == "Save")
				{
					$SAVEMTODTLS	=	"INSERT INTO WMS_NEW.MTO_IB_DTL(`MTONO`,`SKUNO`, `DESCRIPTION`, `ITEMSTATUS`, `QTY`,`NO_OF_BOXES`, `NO_OF_PACK`, `UNITPRICE`, `GROSSAMT`, `RANGESCANFROM`, `RANGESCANTO`)
				...
				{
					$SAVEMTODTLS	=	"UPDATE WMS_NEW.MTO_IB_DTL SET `QTY` = '$txtqty',`UNITPRICE`='$UNITPRICE', `GROSSAMT`='$GROSSAMOUNT', NO_OF_BOXES = '$txtboxes', NO_OF_PACK = '$txtpackages'
	...
	echo "<script>
				MessageType.successMsg('Transaction has been successfully $endmsg.');
...
if($action == "EDITTRX")
{
	...
	else 
	{
		$table	=	"<form id='frmdata'>
						<table border='1' class='tblresult' id='tbltrxnonmto'>
							...
							<tr class='trheader'>
								...
								<td width='8%' align='center'>Item No.</td>
								<td width='46%' align='center'>Description</td>
								<td width='7%' align='center'>Onhand Qty</td>
								<td width='7%' align='center'>Received Qty</td>
								<td width='7%' align='center'>Select</td>
							</tr>";
		$cnt = 1;
		while (!$RSGETTRXDTLS->EOF) {
			...
			$SCANNEDQTY		=	$DATASOURCE->selvalqry($conn_255_10,"SELECT SUM(D.`IB_QTY`) AS IB_QTY FROM WMS_NEW.SCANDATA_DTL AS D LEFT JOIN WMS_NEW.SCANDATA_HDR AS H ON H.MPOSNO = D.MPOSNO WHERE H.POSTEDBY != '' AND D.`DELBY` = '' AND H.POSTEDDATE BETWEEN '$RANGESCANFROM' AND '$RANGESCANTO' AND D.`IB_QTY` != 0 AND D.`SKUNO` = '$SKUNO'","IB_QTY");
			$POSTEDMTOQTY	=	$DATASOURCE->selvalqry($conn_255_10,"SELECT SUM(D.`QTY`) AS QTY FROM WMS_NEW.MTO_IB_DTL AS D LEFT JOIN WMS_NEW.MTO_IB_HDR AS H ON H.MTONO = D.MTONO WHERE H.POSTBY != '' AND (D.RANGESCANFROM BETWEEN '$RANGESCANFROM' AND '$RANGESCANTO' OR D.RANGESCANTO BETWEEN '$RANGESCANFROM' AND '$RANGESCANTO') AND D.`SKUNO` = '$SKUNO' AND H.STATUS != 'CANCELLED'","QTY");
...
if($action == "CANCELTRX")
{
	...
	$conn_255_10->CompleteTrans();
	echo "<script>
				MessageType.infoMsg('Transaction $TRXNO has been successfully cancelled.');
...
if($action == "POSTTRX")
{
	...
	$conn_255_10->CompleteTrans();
	echo "<script>
				MessageType.infoMsg('Transaction $TRXNO has been successfully posted.');
				$('#btnreport').trigger('click',['YES']);
				$('#divtrxmto').dialog('close');
		  </script>";
	exit();
...
if($action == "TRANSMITTRX")
{
	...
	$conn_255_10->CompleteTrans();
	echo "<script>
				MessageType.infoMsg('Transaction $TRXNO has been successfully transmitted.');
				$('#btnreport').trigger('click',['YES']);
		  </script>";
	exit();
}
