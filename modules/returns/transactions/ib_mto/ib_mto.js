function IB_MTO(){}
	IB_MTO.prototype = {
		...
		searchItems:function(){
			...
			{
				$.ajax({
					url			:	"ib_mto.php?action=CREATEIBMTO&txtsfrom="+txtsfrom+"&txtsto="+txtsto,
					beforeSend	:function(){
					...
					success		:function(response){
						...
						$('#divtrxmto').dialog('option', 'title', 'INTERNAL BARCODE RETURNS MTO CREATION');
		...
		saveTrx:function(mode){
			...
			$.ajax({
				data		:frmdata,
				type		:"POST",
				url			:"ib_mto.php?action=SAVETRX&mode="+mode+"&trxno="+trxno,
				beforeSend	:function(){
					$("#divloader").dialog("open");
				},
				success		:function(TRXNO)
				{
					...
					for(var a = 1; a < hidcnt; a++)
					{
						if($("#txtitemno"+a).is(":checked"))
						{
							...
							$.ajax({
								type		:"POST",
								url			:"ib_mto.php?action=SAVETRXDTLS&txtitemno="+txtitemno+"&txtqty="+txtqty+"&mode="+mode+"&TRXNO="+TRXNO+"&a="+a+"&txtboxes="+
		...
		cancelTrx:function(TRXNO){
			 $.ajax({
					type	:	"GET",
					url		:	"ib_mto.php?action=CANCELTRX&TRXNO="+TRXNO,
					beforeSend:	function()
					{
						$("#divloader").dialog("open");
					},
		...
		postTrx:function(TRXNO){
			$.ajax({
					type	:	"GET",
					url		:	"ib_mto.php?action=POSTTRX&TRXNO="+TRXNO,
					beforeSend:	function()
					{
						$("#divloader").dialog("open");
					},
		...
		printTrx:function(TRXNO){
			$.ajax({
				type	:	"GET",
				url		:	"ib_mto.php?action=PRINTTRX&TRXNO="+TRXNO,
				beforeSend:	function()
				{
					$("#divloader").dialog("open");
				},
		...
		transmitTrx:function(TRXNO){
			$.ajax({
					type	:	"GET",
					url		:	"ib_mto.php?action=TRANSMITTRX&TRXNO="+TRXNO,
					beforeSend:	function()
					{
						$("#divloader").dialog("open");
					},
...
$("document").ready(function(){
	...
	$("#btnreport").click(function(usequery){
		...
		if(valid)
		{
			$.ajax({
				data		:dataform,
				type		:"POST",
				url			:"ib_mto.php?action=GETMTO&USESESSIONQUERY="+usequery,
				beforeSend	:function(){
					$("#divloader").dialog("open");
				},
				success		:function(response){
	...
	$("#divrtnmto").on("click",".tdtrxdtls",function(){
		...
		{	
		    $.ajax({
				type	:	"GET",
				url		:	"ib_mto.php?action=VIEWTRXDTLS&TRXNO="+TRXNO+"&COUNT="+COUNT,
				beforeSend:	function()
				{
					$("#divloader").dialog("open");
				},
    ...
    $("#divrtnmto").on("click",".editbtn",function(){
    	var TRXNO	=	$(this).attr("data-trxno");
    	 $.ajax({
				type	:	"GET",
				url		:	"ib_mto.php?action=EDITTRX&TRXNO="+TRXNO,
				beforeSend:	function()
				{
					$("#divloader").dialog("open");
				},
				success	:function(response)
				{
					$("#divtrxmto").html(response);
					$("#divtrxmto").dialog("open");
			    	$('#divtrxmto').dialog('option', 'title', 'INTERNAL BARCODE RETURNS MTO UPDATE');
    ...
    $("#divrtnmto").on("click",".cancelbtn",function(){
    	var TRXNO	=	$(this).attr("data-trxno");
    	MessageType.confirmmsg(IB_MTO_funcs.cancelTrx,"Do you want to cancel this transaction?",TRXNO);
    });
    $("#divrtnmto").on("click",".postbtn",function(){
    	var TRXNO	=	$(this).attr("data-trxno");
    	MessageType.confirmmsg(IB_MTO_funcs.postTrx,"Do you want to post this transaction?",TRXNO);
    });
    $("#divrtnmto").on("click",".printbtn",function(){
    	var TRXNO	=	$(this).attr("data-trxno");
    	MessageType.confirmmsg(IB_MTO_funcs.printTrx,"Do you want to print this transaction?",TRXNO);
    });
    $("#divrtnmto").on("click",".transmitbtn",function(){
    	var TRXNO	=	$(this).attr("data-trxno");
    	MessageType.confirmmsg(IB_MTO_funcs.transmitTrx,"Do you want to transmit this transaction?",TRXNO);
    });
});
