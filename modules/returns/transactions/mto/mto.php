...
if($action == "GETMPOS")
{
	...
	if($CUSTNO_MTO == "")
	{
		$CUSTNO		=	$global_func->Select_val($Filstar_conn,"WMS_NEW","MPOSHDR","CUSTNO","MPOSNO= '{$MPOSNO}' AND MPOSSTATUS = ''");
		if($CUSTNO != "")
		{
...
if($action == "SAVETRX")
{
	...
	else 
	{
		for($a = 1; $a <= $cnt; $a++)
		{
			...
			$GETMPOS	=	"SELECT `SKUNO`,`ITEMSTATUS`, `SCANNEDQTY`, `POSTEDQTY`,`DEFECTIVEQTY`,`IB_QTY` FROM WMS_NEW.SCANDATA_DTL 
			...
			else 
			{
				while (!$RSGETMPOS->EOF) {
					...
					$POSTEDQTY		= $RSGETMPOS->fields["SCANNEDQTY"];
