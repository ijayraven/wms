...
if($action == "CREATEEXMTO")
{
	...
	$txtsto		=	$_GET["txtsto"];
	$GETEXITEMS	=	"SELECT D.`SKUNO`, SUM(D.`SCANNEDQTY` - D.`IB_QTY` - D.`DEFECTIVEQTY`) AS SCANNEDQTY
					 FROM WMS_NEW.SCANDATA_DTL AS D
					 LEFT JOIN WMS_NEW.SCANDATA_HDR AS H ON H.MPOSNO = D.MPOSNO 
					 WHERE D.STATUS = 'POSTED' AND D.`DELBY` = '' AND H.POSTEDDATE BETWEEN '$txtsfrom' AND '$txtsto' AND D.MTOEXCREATED != 'Y' 
					 AND 
					 D.`SKUNO` IN (	SELECT DX.`ITEMNO` FROM WMS_LOOKUP.MTO_EX_ITEMS_DTLS AS DX
									LEFT JOIN WMS_LOOKUP.MTO_EX_ITEMS_HDR AS HX ON HX.TRX_NO = DX.TRX_NO 
									WHERE HX.POSTEDBY != '' AND DX.CANCELLED = 'N'
									GROUP BY DX.ITEMNO	)
					 GROUP BY D.`SKUNO`";
	...
	if($RSGETEXITEMS == false)
	{
		...
		$DATASOURCE->logError("wms",$errmsg,$GETEXITEMS,$_SESSION['username'],"EXCLUSIVE MTO","CREATEEXMTO");
	...
	else
	{
		...
		else 
		{
			...
			while (!$RSGETEXITEMS->EOF)
			{
				...
				$QTY			= 	$RSGETEXITEMS->fields["SCANNEDQTY"];
...
if($action == "SAVETRX")
{
	...
	{
		$TRXNO	=	$_GET["trxno"];
		$SAVEMTOTRX	=	"UPDATE  WMS_NEW.MTO_RTN_EXCHDR SET `STATUS` = 'UPDATED',EDITBY = '$USER', `EDITDATE` ='{$DATE}',EDITTIME='{$TIME}'
						 WHERE MTONO = '$TRXNO'";
		$endmsg	=	"updated";
	}
	$RSSAVEMTOTRX	=	$DATASOURCE->execQUERY("wms",$conn_255_10,$SAVEMTOTRX,$USER,"EXCLUSIVE MTO","SAVETRX");
	if($RSSAVEMTOTRX)
	{
		echo $TRXNO;
	}
...
if($action == "SAVETRXDTLS")
{
	...
	if($txtitemno)
	{
		if($mode == "Save")
		{
			$SAVEMTODTLS	=	"INSERT INTO WMS_NEW.MTO_RTN_EXCDTL(`MTONO`, `SKUNO`, `DESCRIPTION`, `ITEMSTATUS`, `QTY`,`NO_OF_BOXES`, `NO_OF_PACK`,`UNITPRICE`, `GROSSAMT`,`RANGESCANFROM`, `RANGESCANTO`)
								 VALUES('{$TRXNO}','{$txtitemno}','{$ITEMDESC}','{$ITEMTYPE}','{$txtqty}','{$txtboxes}','{$txtpackages}','{$UNITPRICE}','{$GROSSAMOUNT}','$txtsfrom','$txtsto')";
			...
			$RSUPDATEDTL=	$DATASOURCE->execQUERY("wms",$conn_255_10,$UPDATEDTL,$USER,"EXCLUSIVE MTO","SAVETRX");
		}
		else 
		{
			$SAVEMTODTLS	=	"UPDATE WMS_NEW.MTO_RTN_EXCDTL SET NO_OF_BOXES = '$txtboxes', NO_OF_PACK = '$txtpackages'
								 WHERE MTONO = '$TRXNO' AND SKUNO = '$txtitemno'";
		}
		$RSSAVEMTODTLS	=	$DATASOURCE->execQUERY("wms",$conn_255_10,$SAVEMTODTLS,$USER,"EXCLUSIVE MTO","SAVETRX");
		if($RSSAVEMTODTLS)
		{
			echo $a;
		}
...
if($action == "GETMTO")
{
	...
	{
		$errmsg	=	($conn_255_10->ErrorMsg()."::".__LINE__); 
		$DATASOURCE->logError("wms",$errmsg,$GETMTO,$_SESSION['username'],"EXCLUSIVE MTO","GETMTO");
		$DATASOURCE->displayError();
	}
	else 
	{
...
if($action == "VIEWTRXDTLS")
{
	...
	{
		$errmsg	=	($conn_255_10->ErrorMsg()."::".__LINE__); 
		$DATASOURCE->logError("wms",$errmsg,$GETDTLS,$_SESSION['username'],"EXCLUSIVE MTO","VIEWTRXDTLS");
		$DATASOURCE->displayError();
	}
	else 
	{
...
{
	$TRXNO			=	$_GET["TRXNO"];
	$NO_BOXES		=	$DATASOURCE->selval($conn_255_10,"WMS_NEW","MTO_RTN_EXCDTL","NO_OF_BOXES","MTONO = '{$TRXNO}'");
	$NO_PACKAGES	=	$DATASOURCE->selval($conn_255_10,"WMS_NEW","MTO_RTN_EXCDTL","NO_OF_PACK","MTONO = '{$TRXNO}'");
	...
	{
		$errmsg	=	($conn_255_10->ErrorMsg()."::".__LINE__); 
		$DATASOURCE->logError("wms",$errmsg,$GETTRXDTLS,$_SESSION['username'],"EXCLUSIVE MTO","EDITTRX");
		$DATASOURCE->displayError();
	}
	else 
	{
...
if($action == "CANCELTRX")
{
	...
	$CANCELMTOTRX	=	"UPDATE WMS_NEW.MTO_RTN_EXCHDR SET `STATUS` = 'CANCELLED', `CANCELLEDBY` = '{$_SESSION['username']}', `CANCELLEDDT` = '$TODAY'
						 WHERE MTONO = '{$TRXNO}'";
	$RSCANCELMTOTRX	= 	$DATASOURCE->execQUERY("wms",$conn_255_10,$CANCELMTOTRX,$USER,"EXCLUSIVE MTO","CANCELTRX");
	if($RSCANCELMTOTRX)
	{
		$GETDTLS	=	"SELECT `RANGESCANFROM`, `RANGESCANTO`, SKUNO FROM WMS_NEW.MTO_RTN_EXCDTL WHERE MTONO = '$TRXNO'";
		$RSGETDTLS	=	$conn_255_10->Execute($GETDTLS);
		if($RSGETDTLS == false)
		{
			$errmsg	=	($conn_255_10->ErrorMsg()."::".__LINE__); 
			$DATASOURCE->logError("wms",$errmsg,$GETDTLS,$_SESSION['username'],"EXCLUSIVE MTO","CANCELTRX");
			$DATASOURCE->displayError();
		}
		else 
		{
...
if($action == "POSTTRX")
{
	...
	$POSTMTOTRX	=		"UPDATE WMS_NEW.MTO_RTN_EXCHDR SET `STATUS` = 'POSTED', `POSTBY` = '{$_SESSION['username']}', `POSTDATE` = '$TODAY',POSTTIME = '{$time}'
						 WHERE MTONO = '{$TRXNO}'";
	$RSPOSTMTOTRX	= 	$DATASOURCE->execQUERY("wms",$conn_255_10,$POSTMTOTRX,$USER,"EXCLUSIVE MTO","POSTTRX");
	$conn_255_10->CompleteTrans();
	echo "<script>
...
if ($action == "PRINTTRX")
{
	...
	$PRINTMTO	=	"UPDATE WMS_NEW.MTO_RTN_EXCHDR SET `STATUS` = 'PRINTED',`PRINTBY`='{$_SESSION['username']}', `PRINTDATE`='{$TODAY}', `PRINTTIME`='{$time}'
					 WHERE `MTONO` = '{$TRXNO}'";
	$RSPRINTMTO	=	$DATASOURCE->execQUERY("wms",$conn_255_10,$PRINTMTO,$user,"EXCLUSIVE MTO","PRINTTRX");
	if($RSPRINTMTO)
	{
		echo "<script>
...
if($action == "TRANSMITTRX")
{
	...
	$TRANSMITTRX	=	"UPDATE WMS_NEW.MTO_RTN_EXCHDR SET `STATUS` = 'TRANSMITTED', `TRANSMITTED_BY` = '{$_SESSION['username']}', `TRANSMITTED_DT` = '$TODAY'
						 WHERE MTONO = '{$TRXNO}'";
	$RSTRANSMITTRX	= 	$DATASOURCE->execQUERY("wms",$conn_255_10,$TRANSMITTRX,$user,"EXCLUSIVE MTO","TRANSMITTRX");
	if($RSTRANSMITTRX)
	{
		$INSERTTORAW	=	"INSERT INTO WMS_NEW.MTO_RAWHDR(`MTONO`,`STATUS`,`DESTINATION`,`MTO_TRANSMITTED_DT`)
							 VALUES('$TRXNO','','PIECEWORK','$TODAY')";
		$RSINSERTTORAW	=	$DATASOURCE->execQUERY("wms",$conn_255_10,$INSERTTORAW,$user,"EXCLUSIVE MTO","TRANSMITTRX");
		if($RSINSERTTORAW)
		{
			...
			{
				$errmsg	=	($conn_255_10->ErrorMsg()."::".__LINE__); 
				$DATASOURCE->logError("wms",$errmsg,$GETDTLS,$_SESSION['username'],"EXCLUSIVE MTO","TRANSMITTRX");
				$DATASOURCE->displayError();
			}
			else 
			{
				while (!$RSGETDTLS->EOF) {
					...
					$RSINSERTTORAWDTLS	=	$DATASOURCE->execQUERY("wms",$conn_255_10,$INSERTTORAWDTLS,$user,"EXCLUSIVE MTO","TRANSMITTRX");
				$RSGETDTLS->MoveNext();
				}
			}
		}
