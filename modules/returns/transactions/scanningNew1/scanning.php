...
if($action == "VIEWMPOSDTLS")
{
	...
	else 
	{
		...
		while (!$RSGETMPOSDTLS->EOF) {
			...
			$SCANNEDS		=	$DATASOURCE->selval($conn_255_10,"WMS_NEW ","SCANDATA_DTL","ITEMSTATUS","SKUNO = '{$SKUNO}' AND MPOSNO = '{$MPOSNO}'");
			$DEFECTIVEQTY	=	$DATASOURCE->selval($conn_255_10,"WMS_NEW ","SCANDATA_DTL","DEFECTIVEQTY","SKUNO = '{$SKUNO}' AND MPOSNO = '{$MPOSNO}'");
			$GOODQTY		=	$DATASOURCE->selval($conn_255_10,"WMS_NEW ","SCANDATA_DTL","GOODQTY","SKUNO = '{$SKUNO}' AND MPOSNO = '{$MPOSNO}'");
			$IB_QTY			=	$DATASOURCE->selval($conn_255_10,"WMS_NEW ","SCANDATA_DTL","IB_QTY","SKUNO = '{$SKUNO}' AND MPOSNO = '{$MPOSNO}'");
			$SCANNEDQTY		=	$DATASOURCE->selval($conn_255_10,"WMS_NEW ","SCANDATA_DTL","SCANNEDQTY","SKUNO = '{$SKUNO}' AND MPOSNO = '{$MPOSNO}'");
...
if($action == "SCANNING")
{
	...
	else 
	{
		...
		while (!$RSGETMPOSDTLS->EOF) {
			...
			$DEFECTIVEQTY=	$DATASOURCE->selval($conn_255_10,"WMS_NEW","SCANDATA_DTL","DEFECTIVEQTY","SKUNO= '{$SKUNO}' AND MPOSNO = '$MPOSNO'");
			$IB_QTY		=	$DATASOURCE->selval($conn_255_10,"WMS_NEW","SCANDATA_DTL","IB_QTY","SKUNO= '{$SKUNO}' AND MPOSNO = '$MPOSNO'");
			$ADDTL		=	$DATASOURCE->selval($conn_255_10,"WMS_NEW","SCANDATA_DTL","ADDTL","SKUNO= '{$SKUNO}' AND MPOSNO = '$MPOSNO'");
			$STATUS		=	$DATASOURCE->selval($conn_255_10,"WMS_NEW","SCANDATA_DTL","STATUS","SKUNO= '{$SKUNO}' AND MPOSNO = '$MPOSNO'");
			$SCANNEDQTY	=	$DATASOURCE->selval($conn_255_10,"WMS_NEW","SCANDATA_DTL","SCANNEDQTY","SKUNO= '{$SKUNO}' AND MPOSNO = '$MPOSNO'");
			$SCANNEDS	=	$DATASOURCE->selval($conn_255_10,"WMS_NEW","SCANDATA_DTL","ITEMSTATUS","SKUNO= '{$SKUNO}' AND MPOSNO = '$MPOSNO'");
			if($UNITPRICE == '')
			{
...
if($action == "SAVEITEM")
{
	...
	$conn_255_10->StartTrans();
	$SCANNEDQTY		=	$DATASOURCE->selval($conn_255_10,"WMS_NEW","SCANDATA_DTL","SCANNEDQTY","SKUNO= '{$itemno}' AND `MPOSNO` = '$MPOSNO'");
	$GOODQTY		=	$DATASOURCE->selval($conn_255_10,"WMS_NEW","SCANDATA_DTL","GOODQTY","SKUNO= '{$itemno}' AND `MPOSNO` = '$MPOSNO'");
	$DEFECTIVEQTY	=	$DATASOURCE->selval($conn_255_10,"WMS_NEW","SCANDATA_DTL","DEFECTIVEQTY","SKUNO= '{$itemno}' AND `MPOSNO` = '$MPOSNO'");
	$IB_QTY			=	$DATASOURCE->selval($conn_255_10,"WMS_NEW","SCANDATA_DTL","IB_QTY","SKUNO= '{$itemno}' AND `MPOSNO` = '$MPOSNO'");
	...
	$MPOSITEM_FOUND	=	$DATASOURCE->selval($conn_255_10,"WMS_NEW","SCANDATA_DTL","SKUNO","SKUNO= '{$itemno}' AND `MPOSNO` = '$MPOSNO'");
	...
	if($MPOSITEM_FOUND == "")
	{
		...
		$SAVEITEM	=	"INSERT INTO WMS_NEW.SCANDATA_DTL(`MPOSNO`,`CUSTCODE`,`SKUNO`,`ITEMSTATUS`,`BARCODE`, `SCANNEDQTY`,`GOODQTY`, `DEFECTIVEQTY`, `IB_QTY`, `ADDTL`)
						 VALUES('$MPOSNO','$CUSTCODE','$itemno','$ITEMSTATUS','$BARCODE','$NEW_SCANNEDQTY','$GOODQTY','$DEFECTIVEQTY','$IB_QTY','$ADDEDITEM')";
	}
	else 
	{
		$SAVEITEM	=	"UPDATE WMS_NEW.SCANDATA_DTL SET `STATUS`='UPDATED',`SCANNEDQTY`='$NEW_SCANNEDQTY',`GOODQTY`='$GOODQTY', `DEFECTIVEQTY`='$DEFECTIVEQTY', `IB_QTY`='$IB_QTY',
	...
	$conn_255_10->CompleteTrans();
	if($RSSAVEITEM and $UPDATESTATUS)
	{
...
if($action == "SAVEITEM_REPLACEQTY")
{
	...
	$adddtl		=	$_GET["adddtl"];
	$conn_255_10->StartTrans();
	$SCANSTATUS	=	$DATASOURCE->selval($conn_255_10,"FDCRMSlive","itemmaster","DeptNo","ItemNo= '{$hiditemno}'");

	$SAVEITEM		=	"UPDATE WMS_NEW.SCANDATA_DTL SET `STATUS`='UPDATED',`SCANNEDQTY`='$txtrecqty',`GOODQTY`='$txtgoodqty', `DEFECTIVEQTY`='$txtdefqty', `IB_QTY`='$txtibqty',ITEMSTATUS='$SCANSTATUS',
						`UPDATEDBY`='$user', `UPDATEDDATE`='$day', `UPDATEDTIME`='$time'
						 WHERE `MPOSNO` = '$MPOSNO' AND `SKUNO` = '$hiditemno'";
	$RSSAVEITEM		=	execQUERYhere("wms",$conn_255_10,$SAVEITEM,$user,"MPOS SCANNING","SAVEITEM");
	
	$UPDATESTATUS	=	"UPDATE WMS_NEW.SCANDATA_HDR SET `STATUS`='UPDATED' WHERE `MPOSNO` = '$MPOSNO'";
	$RSUPDATESTATUS	=	execQUERYhere("wms",$conn_255_10,$UPDATESTATUS,$user,"MPOS SCANNING","SAVEITEM");
	$conn_255_10->CompleteTrans();
	if($RSSAVEITEM == false or $RSUPDATESTATUS == false)
	{
