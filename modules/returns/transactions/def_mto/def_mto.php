...
if($action == "CREATEEXMTO")
{
	...
	$txtsto		=	$_GET["txtsto"];
	$GETEXITEMS	=	"SELECT D.MPOSNO, D.`SKUNO`, D.`ITEMSTATUS`, SUM(D.`DEFECTIVEQTY`) AS QTY, D.`DELBY`,D.MTODEFCREATED FROM WMS_NEW.SCANDATA_DTL AS D
					 LEFT JOIN WMS_NEW.SCANDATA_HDR AS H ON H.MPOSNO = D.MPOSNO 
					 WHERE H.POSTEDBY != '' AND D.`DELBY` = '' AND H.POSTEDDATE BETWEEN '$txtsfrom' AND '$txtsto'
					 AND D.MTODEFCREATED = 'N' AND D.`DEFECTIVEQTY` != 0
					 GROUP BY D.`SKUNO`";
...
if($action == "SAVETRXHDR")
{
	...
	$SAVEMTOTRX	=	"INSERT INTO WMS_NEW.MTO_DEFECTIVE_HDR(`MTONO`, `STATUS`, `DESTINATION`, `ADDBY`, `ADDDATE`, `ADDTIME`)
	...
	else 
	{
		...
		else
		{
			while (!$RSGETEXITEMS->EOF)
			{
				...
				$ITEMSTATUS		= 	$RSGETEXITEMS->fields["ITEMSTATUS"];
				$POSTEDQTY		= 	$RSGETEXITEMS->fields["QTY"];
				$ITEMDESC		=	addslashes($DATASOURCE->selval($conn_250_171,"FDC_PMS","ITEMMASTER","ITEM_DESC","ITEMNO = $ITEMNO"));
				$ONHAND			=	$POSTEDQTY;
				if($ONHAND > 0)
				{
					$ITEMTYPE 	= $DATASOURCE->selval($conn_255_10,"FDCRMSlive","itemmaster","DeptNo","ItemNo= '{$ITEMNO}'");
					$UNITPRICE 	= $DATASOURCE->selval($conn_255_10,"FDCRMSlive","itemmaster","UnitPrice","ItemNo = '{$ITEMNO}'");
					$GROSSAMOUNT= $POSTEDQTY * $UNITPRICE;
					$SAVEMTODTLS	=	"INSERT INTO WMS_NEW.MTO_DEFECTIVE_DTL(`MTONO`,`SKUNO`, `DESCRIPTION`, `ITEMSTATUS`, `QTY`,`NO_OF_BOXES`, `NO_OF_PACK`, `UNITPRICE`, `GROSSAMT`, `RANGESCANFROM`, `RANGESCANTO`)
										 VALUES('{$TXNO}','{$ITEMNO}','{$ITEMDESC}','{$ITEMTYPE}','{$ONHAND}','{$txtnoboxes}','{$txtnopackages}','{$UNITPRICE}','{$GROSSAMOUNT}','$txtsfrom','$txtsto')";
					...
					else 
					{
						$UPDATEDTL 	=	"UPDATE WMS_NEW.SCANDATA_DTL AS D
										 LEFT JOIN WMS_NEW.SCANDATA_HDR AS H ON H.MPOSNO = D.MPOSNO
										 SET D.MTODEFCREATED = 'Y' 
										 WHERE H.POSTEDDATE BETWEEN '$txtsfrom' AND '$txtsto' AND D.SKUNO = '$ITEMNO'";
						$RSUPDATEDTL=	$DATASOURCE->execQUERY("wms",$conn_255_10,$UPDATEDTL,$_SESSION['username'],"DEFECTIVE STOCKS MTO","SAVETRXDTLS");
					}
				}
				$RSGETEXITEMS->MoveNext();
			}
...
//	}
	...
	{
		$GETMTO	=	"SELECT * FROM WMS_NEW.MTO_DEFECTIVE_HDR
...
if($action == "VIEWTRXDTLS")
{
	$TRX_NO		=	$_GET["TRXNO"];
	$COUNT		=	$_GET["COUNT"];
	$GETDTLS	=	"SELECT * FROM WMS_NEW.MTO_DEFECTIVE_DTL WHERE MTONO = '{$TRX_NO}' GROUP BY SKUNO";
...
{
	$TRXNO			=	$_GET["TRXNO"];
	$NO_BOXES		=	$DATASOURCE->selval($conn_255_10,"WMS_NEW","MTO_DEFECTIVE_DTL","NO_OF_BOXES","MTONO = '{$TRXNO}'");
	$NO_PACKAGES	=	$DATASOURCE->selval($conn_255_10,"WMS_NEW","MTO_DEFECTIVE_DTL","NO_OF_PACK","MTONO = '{$TRXNO}'");
	$GETTRXDTLS		=	"SELECT * FROM  WMS_NEW.MTO_DEFECTIVE_DTL WHERE MTONO = '{$TRXNO}'";
...
if($action == "UPDATETRX")
{
	...
	$time			=	date("H:i:s A");
	$conn_255_10->StartTrans();
	$UPDATEMTOTRX	=	"UPDATE  WMS_NEW.MTO_DEFECTIVE_HDR SET `STATUS` = 'UPDATED',EDITBY = '{$_SESSION['username']}', `EDITDATE` ='{$TODAY}',EDITTIME='{$time}'
						 WHERE MTONO = '{$TRX_NO}'";
	...
	{
		$UPDATEDTL	=	"UPDATE WMS_NEW.MTO_DEFECTIVE_DTL SET NO_OF_BOXES = '$txtnoboxes', NO_OF_PACK = '$txtnopackages'
						 WHERE MTONO = '$TRX_NO'";
		$RSUPDATEDTL=	$DATASOURCE->execQUERY("wms",$conn_255_10,$UPDATEDTL,$_SESSION['username'],"DEFECTIVE RETURNS MTO","UPDATETRX");
	}
	$conn_255_10->CompleteTrans();
	echo "<script>
...
if($action == "POSTTRX")
{
	...
	$conn_255_10->StartTrans();
	$POSTMTOTRX	=		"UPDATE WMS_NEW.MTO_DEFECTIVE_HDR SET `STATUS` = 'POSTED', `POSTBY` = '{$_SESSION['username']}', `POSTDATE` = '$TODAY',POSTTIME = '{$time}'
						 WHERE MTONO = '{$TRXNO}'";
...
if($action == "CANCELTRX")
{
	...
	$conn_255_10->StartTrans();
	$CANCELMTOTRX	=		"UPDATE WMS_NEW.MTO_DEFECTIVE_HDR SET `STATUS` = 'CANCELLED', `CANCELLEDBY` = '{$_SESSION['username']}', `CANCELLEDDT` = '$TODAY'
							 WHERE MTONO = '{$TRXNO}'";
	...
	{
		$GETDTLS	=	"SELECT `RANGESCANFROM`, `RANGESCANTO`, SKUNO FROM WMS_NEW.MTO_DEFECTIVE_DTL WHERE MTONO = '$TRXNO'";
		$RSGETDTLS	=	$conn_255_10->Execute($GETDTLS);
		if($RSGETDTLS == false)
		{
			echo $conn_255_10->ErrorMsg()."::".__LINE__; exit();
...
if($action == "TRANSMITTRX")
{
	...
	$conn_255_10->StartTrans();
	$TRANSMITTRX	=	"UPDATE WMS_NEW.MTO_DEFECTIVE_HDR SET `STATUS` = 'TRANSMITTED', `TRANSMITTED_BY` = '{$_SESSION['username']}', `TRANSMITTED_DT` = '$TODAY'
						 WHERE MTONO = '{$TRXNO}'";
	...
	else 
	{
		...
		{
			$GETDTLS	=	"SELECT * FROM WMS_NEW.MTO_DEFECTIVE_DTL WHERE MTONO = '{$TRXNO}'";
			$RSGETDTLS	=	$conn_255_10->Execute($GETDTLS);
			if($RSGETDTLS == false)
			{
				echo $conn_255_10->ErrorMsg()."::".__LINE__; exit();
...
{
	$forTRXno		=	"SELECT	MTONO,ADDDATE FROM  WMS_NEW.MTO_DEFECTIVE_HDR order by LINE_NO";
	...
	while (!$rsforTRXno->EOF) 
	{
	 ...
	 if( $date1==$date2 )
	 {
		if($lnt == 1)
		{
			$newTRXno	=	"DRTN-".date('Ymd').'-'."00".$newdgt;
		}
		if($lnt	==	2)
		{
			$newTRXno	=	"DRTN-".date('Ymd').'-'."0".$newdgt;
		}
		if($lnt	==	3)
		{
			$newTRXno	=	"DRTN-".date('Ymd').'-'.$newdgt;
		}
	 }
	 else 
	 {
	 	$newTRXno	=	"DRTN-".date('Ymd').'-'."001";
	 }
...
include($_SERVER['DOCUMENT_ROOT']."/wms/modules/returns/transactions/def_mto/def_mto.html");
include($_SERVER['DOCUMENT_ROOT']."/wms/modules/returns/transactions/def_mto/def_mtoUI.php");
?>
