...
//$sel_h		=	"SELECT `SOF`,`CUSTNO` FROM WMS_NEW.CONFIRMDELIVERY_HDR WHERE SOF in ({$sofno}) ";
$sel_h		=	"SELECT `SOF`,`CUSTNO` FROM WMS_NEW.CONFIRMDELIVERY_HDR WHERE SOF in ('{$sofno}') ";
...
while (!$rssel_h->EOF) 
{
	$SOF	=	$rssel_h->fields['SOF'];
	$CUSTNO	=	$rssel_h->fields['CUSTNO'];
	
	$sel_d	=	"SELECT * from WMS_NEW.CONFIRMDELIVERY_DTL WHERE SOF = '{$SOF}' ";
	...
	while (!$rssel_d->EOF) 
	{
		$SKUNO	=	$rssel_d->fields['SKUNO'];
		$QTY	=	$rssel_d->fields['RECEIVEDQTY'];
		...
		//$DISC			=	100-$DISC1;
		$DISC        = AccntRound($DISC1 / 100, 2);
		$DiscAmt     = AccntRound(($UnitPrice * $DISC), 2);
		$UnitCost	 =	AccntRound($UnitPrice-$DiscAmt,2);
		
		$net			=	AccntRound($UnitCost*$ReleaseQty,2);
		$net_hdr		=	AccntRound($UnitCost*$QTY,2);
		...
		$update_dtl 	=	"UPDATE orderdetail set UnitCost = '{$UnitCost}', Discount = '{$DISC1}', NetAmount = '{$net}' where OrderNo = '{$SOF}' and Item = '{$SKUNO}'";
	...
	$UPDATE_HDR		=	"UPDATE WMS_NEW.CONFIRMDELIVERY_HDR SET RCVDNETAMOUNT = '{$total_net}' , RCVDTOTALQTY = '{$total_qty}' WHERE SOF = '{$SOF}' ";
