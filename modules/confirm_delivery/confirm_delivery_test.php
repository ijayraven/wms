...
//		if ($confirmed == 0) 
//		{
			$sel_data	=	"SELECT OrderNo,CustNo,InvoiceNo,InvoiceAmount from orderheader where InvoiceNo = '{$THIS_VAL}' and OrderCategory = '{$OPT}' ";
...
//		}
		...
		foreach ($_SESSION['ITEM'] as $key=>$val_item) 
		{
			...
			if ($del_qty!=$rvc_qty) 
			{
				if (!empty($item)) 
				{
					$insert_dtl	=	"INSERT INTO WMS_NEW.CONFIRMDELIVERY_DTL_test(`DOCNO`,`SOF`,`IATRANSNO`,`SKUNO`,`QTY`,`RECEIVEDQTY`,`REMARKS`)
									VALUES
					...
					$insert_IA	=	"INSERT INTO WMS_NEW.INVENTORYADJUSTMENT_DTL_test(`IATRANSN0`,`SKUNO`,`IAQTY`,`REMARKS`) 
									VALUES
			...
			else 
			{
				if (!empty($item)) 
				{
					$insert_dtl	=	"INSERT INTO WMS_NEW.CONFIRMDELIVERY_DTL_test(`DOCNO`,`SOF`,`SKUNO`,`QTY`,`RECEIVEDQTY`,`REMARKS`)
									VALUES
		...
		$insert_hdr		=	"INSERT INTO WMS_NEW.CONFIRMDELIVERY_HDR_test(`SOF`,`DOCNO`,`CUSTNO`,`SRNO`,`DOCTYPE`,`NETAMOUNT`,`GROSSAMOUNT`,`TOTALQTY`,`IATRANSNO`,
		...
		if ($IA=='YES')
		{
			$insert_ia_hrd	=	"INSERT INTO WMS_NEW.INVENTORYADJUSTMENT_HDR_test(`IATRANSNO`,`CUSTNO`,`REFN0`,`REFTYPE`,`STATUS`,`ADDEDDATE`,`ADDEDTIME`)
...
function	GET_DATA(evt,val_id,val_)
{
	...
	if(evtHandler == 13)
	{
		...
		if(val_ != '')
		{
			...
			$.ajax({
					url			:	'confirm_delivery_test.php?action=GETDATA&OPT='+opt+'&VAL_='+val_+'&OPT2='+opt2,
					beforeSend	:	function()
								{
	...
	if(isconfirm==true)
	{
		...
		if(doprocess == 'YES')
		{
			...
			for(var first_cnt=1;first_cnt<=total_cnt;first_cnt++)
			{
				...
				$.ajax({
						url		:	'confirm_delivery_test?action=SET_ITEMS&ITEM='+item+'&DEL_QTY='+del_qty+'&REL_QTY='+rel_qty+'&NEW_GROSS_='+new_gross_+'&OLD_GROSS_='+old_gross_+'&NEW_NET_='+new_net_+'&OLD_NET_='+old_net_+'&REMARKS_='+remarks_+'&HDN_REMARKS_='+hdn_remarks_,
						async	: 	false,
						success	:	function(response)
								{
...
//			$.ajax({
//					type		:	'POST',
//					url			:	'confirm_delivery_test?action=CONFIRM__&VAL_OPT='+val_opt+'&VAL__='+val__+'&TOTAL_CNT='+total_cnt+'&DATEDEL='+dateDEL+'&AMT_DISCREPANCY='+amt_discrepancy,
//					beforeSend	:	function()
//								{
//									$('#divloader').dialog('open');
//								},
...
function	Display_item(val_opt,val__)
{
	$('#div_detail').show();
//	$.ajax({
//			url			:	'confirm_delivery_test?action=GET_ITEM&OPT='+val_opt+'&VAL_='+val__,
//			beforeSend	:	function()
//						{
//							$('#divloader').dialog('open');
//						},
...
{
	$.ajax({
			url		:	'confirm_delivery_test?action=GET_IA&VAL_OPT='+val_opt+'&VAL__='+val__,
			success	:	function(response)
					{
						alert(response);
						if(response != 'none')
...
function recompute(val_item)
{
	...
	{
		$.ajax({
				url		:	'confirm_delivery_test?action=DOCOMPUTE&RECEIVED='+Received+'&UNITCOST='+Unitcost+'&UNITPRICE='+Unitprice,
				success	:	function(response)
						{
...
//		$.ajax({
//				url		:	'confirm_delivery_test?action=DOCOMPUTE2&RECEIVED='+Received+'&UNITPRICE='+Unitprice,
//				success	:	function(response)
//						{
