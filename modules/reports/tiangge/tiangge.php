	...
	if($action=='DISPLAY__')
	{
		...
		while (!$rssel_transno->EOF) 
		{
			...
			while (!$rssel_dtl->EOF) 
			{
				...
				$CLASS_			=	$rssel_dtl->fields['CLASS'];
				
				$aData[$SKUNO]['QTY']			+=	$QTY;
				$aData[$SKUNO]['BRAND']			=	$BRAND_;
				$aData[$SKUNO]['CLASS']			=	$CLASS_;
...
<script>
	...
	function	DISPLAY_()
	{
		...
		$.ajax({
				url			:	'tiangge.php?action=DISPLAY__&BRAND='+brand+'&DFROM='+dfrom+'&DTO='+dto+'&SOURCE_='+source_+'&DESTINATION_='+destination_+'&SEL_STATUS='+sel_status+'&ITEM_STAT='+item_stat+"&CLASS="+selclass,
				beforeSend	:	function()
							{
								$('#loading').dialog('open');
							},
					...
					<table border="0"class="label_text">
						...
						<tr>
							...
							<td>
								...
								$RSGETCLASS	=	$Filstar_pms->Execute($GETCLASS);
								if($RSGETCLASS == false)
								{
									echo $errmsg	=	($Filstar_pms->ErrorMsg()."::".__LINE__);  exit();
								}
								?>
...
$("#divresponse").dialog({
	...
	buttons: {
		...
		'PRINT': function()
		{
			...
			var url	=	'tiangge_PDF.php?action=PDF&BRAND='+brand+'&DFROM='+dfrom+'&DTO='+dto+'&SOURCE_='+source_+'&DESTINATION_='+destination_+'&SEL_STATUS='+sel_status+'&ITEM_STAT='+item_stat+"&CLASS="+selclass;
			window.open(url);
		}
	}
});
