...
//					if($sel_seasons == "X")
//					{
//						$SEASON_Q	=	" AND (SOF REGEXP '$sel_seasons' AND SOF NOT REGEXP 'XN' AND SOF NOT REGEXP 'XL')";
//					}
//					if($sel_seasons == "M")
//					{
//						$SEASON_Q	=	" AND (SOF REGEXP '$sel_seasons' AND SOF NOT REGEXP 'ML')";
//					}
//					if($sel_seasons == "F")
//					{
//						$SEASON_Q	=	" AND (SOF REGEXP '$sel_seasons' AND SOF NOT REGEXP FL')";
//					}
//					if($sel_seasons == "H")
//					{
//						$SEASON_Q	=	" AND (SOF REGEXP '$sel_seasons' AND SOF NOT REGEXP HL')";
//					}
		...
		$sel_val_cust	 =	"SELECT * from WMS_NEW.CONFIRMDELIVERY_HDR WHERE 1 $SEASON_Q ";
	...
	<form name="dataform" id="dataform" enctype="multipart/form-data" method="POST">
		<table width="100%" border="0" class="Text_header" style="border:0px">
			...
			<tr>
				...
				<td align="left" width="57%" style="font-size:15px">
					<!--:<select id="sel_class" name="sel_class" onchange="getseasonal();">-->
			...
			<tr>
				...
				<td align="left" width="57%" style="font-size:15px">
					:<input type="radio" name="rd_ordertype" id="rdstf" value="STF" checked>STF
					&nbsp;<input type="radio" name="rd_ordertype" id="rdinvoice" value="INVOICE">INVOICE
				</td>
			</tr>
			<tr>
				<td width="100%" colspan="3" align="center">
					<input type="button" name="btnsearch" id="btnsearch" value="CSV" class="small_button" onclick="DISPLAY();"  style='width:120px;height:35px;'>
				</td>
			</tr>
		</table>
		</div>
...
function DISPLAY()
{
	var customer_type	=	$('#rdnbs').is(":checked");
	var customercode	=	$('#customercode').val();
	var sel_class		=	$('#sel_class').val();
	var added_date		=	$('#added_date').is(":checked");
	var dfrom			=	$('#dfrom').val();
	var dto				=	$('#dto').val();
	var order_type		=	$('#rdstf').is(":checked");
	...
	var	url_	=	'details_csv.php?action=DL_CSV&OPT_1='+opt_1+'&OPT_2='+opt_2+'&OPT_3='+opt_3+'&CUSTOMERCODE='+customercode+'&SEL_CLASS='+sel_class+'&DFROM='+dfrom+'&DTO='+dto;
	window.open(url_);
}
