...
while (!$rssel_transno->EOF) 
{
	...
	if ($_SESSION['BRAND'] != 'ALL')
	{
		$sel_dtl	=	"SELECT  * FROM WMS_NEW.TIANGGE_DTL WHERE TRANSNO = '{$transno}' AND BRAND = '{$_SESSION['BRAND_NAME']}' ";
		if ($_SESSION['BRAND'] != 'ALL') 
		{
	...
	$rssel_transno->MoveNext();
		...
		class PDF extends FPDF 
		{
			...
			function Header($header_)
			{
				...
				$this->SetX(125);$this->Cell(20,5,'TOTAL QTY',0,0,'C');
				$this->SetX(148);$this->Cell(20,5,'PROD COST',0,0,'C');
				$this->SetX(170);$this->Cell(20,5,'SRP',0,0,'C');
...
//			}
			...
			foreach ($aData as $key_item=>$val__)
			{
				...
				$pdf->SetFont('Courier','I',10);
				$pdf->SetX(35);$pdf->Cell(95,5,substr($item_desc,0,40),0,0,'L');
				$pdf->SetFont('Courier','B',10);
				$pdf->SetX(125);$pdf->Cell(20,5,$QTY,0,0,'C');
				$pdf->SetX(145);$pdf->Cell(20,5,number_format($PRODCOST,2),0,0,'R');
				$pdf->SetX(165);$pdf->Cell(20,5,number_format($RETAILPRICE,2),0,0,'R');
				$pdf->SetX(180);$pdf->Cell(30,5,number_format($COSTAMOUNT,2),0,1,'R');
				$COUNT++;
			}
			$pdf->SetX(125);$pdf->Cell(20,5,$total_qty,0,0,'C');
