<?php
	 ...
	 * FOR SOF
	 */
	//$sel_order_dtl_sof	=	"SELECT CUSTNO,SUM(ORDERQTY)as QTY,SUM(GROSSAMOUNT)as GROSS,SUM(NETAMOUNT)as NET FROM BTS_2016.$dtl WHERE CUSTNO IN ('{$custlist}') GROUP BY CUSTNO ";
	if ($_GET['OPT2'] != 'ALL')
	{
		$sel_order_hdr_sof	=	"SELECT ORDERNO FROM BTS_2016.$hdr WHERE ORDERSTATUS='For Picking' AND CUSTNO in ('{$custlist}')";
	}
	else {
		$sel_order_hdr_sof	=	"SELECT ORDERNO FROM BTS_2016.$hdr WHERE ORDERSTATUS='For Picking' ";
	}
	...
	$sel_order_dtl_sof	=	"SELECT CUSTNO,SUM(ORDERQTY)as QTY,SUM(GROSSAMOUNT)as GROSS,SUM(NETAMOUNT)as NET FROM BTS_2016.$dtl WHERE ORDERNO IN('{$SOFLIST}') GROUP BY CUSTNO ";
	...
	if ($_GET['OPT2'] != 'ALL') 
	{
		$sel_order_172		=	"SELECT ORDERNO FROM BTS_2016.$hdr WHERE ORDERSTATUS = 'For Picking' and PICKLISTNO != '' AND CUSTNO in ('{$custlist}') ";
	}
	else 
	{
		$sel_order_172		=	"SELECT ORDERNO FROM BTS_2016.$hdr WHERE ORDERSTATUS = 'For Picking' and PICKLISTNO != '' ";
	}
