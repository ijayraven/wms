<?php
	 ...
	 * FOR SOF
	 */
	//$sel_order_dtl_sof	=	"SELECT CUSTNO,SUM(ORDERQTY)as QTY,SUM(GROSSAMOUNT)as GROSS,SUM(NETAMOUNT)as NET FROM VALENTINE_2016.$dtl WHERE CUSTNO IN ('{$custlist}') GROUP BY CUSTNO ";
	if ($_GET['OPT2'] != 'ALL')
	{
	...
	$sel_order_dtl_sof	=	"SELECT CUSTNO,SUM(ORDERQTY)as QTY,SUM(GROSSAMOUNT)as GROSS,SUM(NETAMOUNT)as NET FROM VALENTINE_2016.$dtl WHERE ORDERNO IN('{$SOFLIST}') GROUP BY CUSTNO ";
