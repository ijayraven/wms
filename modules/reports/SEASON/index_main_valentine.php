...
//			$sr___name	=	$global_func->Select_val($Filstar_conn,"FDCRMSlive","salesreps","SalesRepName","SalesRepCode = '{$sr__}' ");
	...
	if ($wave=='WAVE1')
	{
		$range	=	date("Y-m-d",strtotime("2016-11-14")).' to '.date("Y-m-d",strtotime("2016-11-14"));
	}
	else if($wave=='WAVE2')
	{
...
//		});
		...
		var	url_	=	'index_main_valentine.php?action='+opt__+'&SEL_YEAR='+sel_year+'&SEL_WAVE='+sel_wave+'&SRCODE='+srcode+'&SEL_PRODUCTTYPE='+sel_producttype+'&CUSTCODE='+custcode;
		window.open(url_);
	}
	...
	function	selsr(evt,this_id)
	{
		...
		if(evthandler != 40 && evthandler != 13 && evthandler != 27)
		{
			...
			{
				$.ajax({
						url			:	'index_main_valentine.php?action=FIND_SR&OPT__='+opt__+'&OPT_VAL='+opt_val,
						beforeSend	:	function()
									{
										$('#loading').dialog('open');
									},
	...
	function	selcust(evt,this_id)
	{
		...
		if(evthandler != 40 && evthandler != 13 && evthandler != 27)
		{
			...
			{
				$.ajax({
						url			:	'index_main_valentine.php?action=FIND_CUST&OPT__='+opt__+'&OPT_VAL='+opt_val+'&CUST_TYPE='+cust_type,
						beforeSend	:	function()
									{
										$('#loading').dialog('open');
									},
