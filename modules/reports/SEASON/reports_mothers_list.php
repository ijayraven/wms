<?php
	...
	$TOTAL4	=	0;
	$TOTAL5	=	0;
	$cCSV	.=	"INVOICE FORMAT;CUSTOMER;SOFNO;ORDERDATE;PLNO;PL DATE;INVNO;TOTAL RELEASE QTY; REC;GROSS;NET;STATUS;CONFIRMED DELIVERY"."\n";
	foreach ($aSOF as $key_sof)
	{
		...
		if($rssel_hdr->RecordCount() > 0)
		{
			while (!$rssel_hdr->EOF) 
			{
				...
				$sel_sof		=	"SELECT SOF FROM WMS_NEW.CONFIRMDELIVERY_HDR WHERE SOF = '{$key_sof}' ";
				$rssel_sof		=	$Filstar_conn->Execute($sel_sof);
				if ($rssel_sof==false) 
				{
					echo $Filstar_conn->ErrorMsg()."::".__LINE__;exit();
				}
				$record_cnt		=	$rssel_sof->RecordCount();
				if ($record_cnt>0) 
				{
					$delivery_cnt	=	"YES";
				}
				...
				$cCSV	.=	"$OrderCategory;$CustNo-$CustName;$key_sof;$OrderDate;$PickListNo;$PickListDate;$InvoiceNo;$QTY2;$count;$GROSS;$NET;$OrderStatus;$delivery_cnt;";
				$cCSV	.=	"\n";
