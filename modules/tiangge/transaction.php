...
if (empty($_SESSION['username'])) 
{
	...
	if ($action=='DISPLAY')
	{
		...
		if ($cnt > 0)
		{
			...
			while (!$rssel_transno->EOF) 
			{
				...
				$show	.=	"<td align='center'  onclick=display_item('{$TRANSNO}')>";
				$show	.=	$TRANSNO;
				$show	.=	"</td>";
				...
				$show	.=	"<td align='center' onclick=display_item('{$TRANSNO}') nowrap>";
				$show	.=	$this_type;
				$show	.=	"</td>";
				...
				if ($_SESSION['username']!='rmpwh')
				{
					...
					$show	.=		"<input type='button' name='btnpcancel' id='btnpcancel' class='small_button' style='width:90px' value='CANCEL' onclick=DOACTION('CANCEL','{$TRANSNO}');>";
					}
	...
	if ($action=='DODELIVERY') 
	{
		...
		$TIANGGE_SKU	=	$_GET['TIANGGE_SKU'];
		
		$Filstar_loc->StartTrans();
		
		$ppoint		=	number_format($PRICEPOINT,2);
		
		$sel_cnt 	=	"SELECT * FROM WMS_LOOKUP.TIANGGE_SKU WHERE SKUNO = '{$TIANGGE_SKU}' AND PRICEPOINT = '{$ppoint}' ";
		$rssel_cnt	=	$Filstar_loc->Execute($sel_cnt);
		...
		if ($rssel_cnt->RecordCount() > 0) 
		{
			$update		=	"UPDATE WMS_NEW.TIANGGE_HDR SET STATUS = 'DELIVERY', CUSTCODE = '{$CUSTCODE}', DRNO = '{$TXTDRNO}',PRICEPOINT = '{$PRICEPOINT}' ,
							DELIVERYBY = '{$_SESSION['username']}', DELIVERYDATE = SYSDATE(),DELIVERYTIME = SYSDATE(), TIANGGESKU = '{$TIANGGE_SKU}' 
		 					WHERE TRANSNO = '{$TRXNO}' ";
	...
	if ($action=='DOPOSTING') 
	{
		...
		{
			$UPDATE 	=	"UPDATE WMS_NEW.TIANGGE_HDR SET STATUS = 'CANCELLED',CANCELLEDBY = '{$_SESSION['username']}',CANCELLEDDATE=SYSDATE(),CANCELLEDTIME=SYSDATE() 
							WHERE  TRANSNO = '{$trxno}' ";
			$rsupdate	=	$Filstar_loc->Execute($UPDATE);
			if ($rsupdate==false) 
			{
		...
		elseif ($_GET['THIS_ACTION']=='CANCEL')
		{
			$UPDATE 	=	"UPDATE WMS_NEW.TIANGGE_HDR SET STATUS = 'CANCELLED',CANCELLEDBY = '{$_SESSION['username']}',CANCELLEDDATE=SYSDATE(),CANCELLEDTIME=SYSDATE() 
							WHERE  TRANSNO = '{$trxno}' ";
			$rsupdate	=	$Filstar_loc->Execute($UPDATE);
	...
	if ($action=='GETTIANGGE') 
	{
		$PRICEPOINT	=	$_GET['VAL__'];
		
		$ppoint		=	number_format($PRICEPOINT,2);
		
		$sel_cnt 	=	"SELECT SKUNO FROM WMS_LOOKUP.TIANGGE_SKU WHERE PRICEPOINT = '{$ppoint}' ";
		$rssel_cnt	=	$Filstar_loc->Execute($sel_cnt);
		if ($rssel_cnt==false) 
		{
			echo $Filstar_loc->ErrorMsg()."::".__LINE__;exit();
		...
		<div id="dialog_delivery" title="DELIVERY">
			<table width="100%" border="1"  class="Text_header">
				...
				<tr>
					...
					<td width="70%">
						<input type="text" name="txtpricepoint" id="txtpricepoint" value="" size="30" onkeyup="GET_TIANGGE(event,this.value);">
					</td>
				</tr>
				<tr>
					<td width="20%" nowrap>
					...
					<td width="70%">
						<input type="text" name="txttiangge_sku" id="txttiangge_sku" value="" size="30" readonly>
					</td>
				</tr>
			</table>
		</div>
...
//		alert("MAINTENANCE -- "+val_traxno);return;
	...
	{
		if(this_action == 'CANCEL')
		{
			var isconfirm	=	confirm('Are you sure you want to cancel this transaction?');
		}
		else
		{
		...
		{
			$.ajax({
					url			:	'transaction.php?action=DOPOSTING&THIS_TRXNO='+this_trxno+'&THIS_ACTION='+this_action,
					beforeSend	:	function()
								{
									$('#divloader').show();
								},
...
$("#dialog_delivery").dialog({
	dialogClass: "no-close",
	bgiframe:true, resizable:false, height: 190, width: 450, modal:true, autoOpen: false,	draggable: false,
	...
	buttons: {
		...
		'SUBMIT': function()
		{
			...
			var tiangge_sku	=	$('#txttiangge_sku').val();
			if(custcode != '' && txtdrno != '')
			{
				$.ajax({
						url			:	'transaction.php?action=DODELIVERY&CUSTCODE='+custcode+'&TXTDRNO='+txtdrno+'&TRXNO='+trxno+'&PRICEPOINT='+pricepoint+'&TIANGGE_SKU='+tiangge_sku,
						beforeSend	:	function()
									{
										$('#divloader').show();
									},
...
function	GET_TIANGGE(evt,val__)
{
	var evtHandler	=	(evt.charCode) ? evt.charCode	: evt.keyCode;
	if(evtHandler==13 && val__ != '')
	{
		$.ajax({
				url		:	'transaction.php?action=GETTIANGGE&VAL__='+val__,
				success	:	function(response)
						{
							if(response=='NONE')
							{
								alert('Invalid price point');
							}
							else
							{
								$('#txttiangge_sku').val(response);
							}
						}
		});
		$('#txttiangge_sku').focus();
	}
}
