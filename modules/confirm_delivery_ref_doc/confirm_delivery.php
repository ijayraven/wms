...
if (empty($_SESSION['username'])) 
{
	...
	if ($action=='GETDATA') 
	{
		...
		$isconfirm	=	"SELECT SOF from WMS_NEW.CONFIRMDELIVERY_HDR WHERE DOCNO = '{$THIS_VAL}' AND DOCTYPE = 'RECEIVING FORM' ";
		...
		if ($confirmed == 0) 
		{
			$sel_data	=	"SELECT SOF,CUSTCODE,NETAMOUNT from WMS_NEW.RECEIVINGFORM_HDR where RFNO = '{$THIS_VAL}' ";
			...
			if ($record > 0) 
			{
			$OrderNo	=	$rssel_data->fields['SOF'];
			$CustNo		=	$rssel_data->fields['CUSTCODE'];
			//$InvoiceNo	=	$rssel_data->fields['InvoiceNo'];
			$NetAmount	=	$rssel_data->fields['NETAMOUNT'];
			
			if ($OPT2	== 'MANILA')
			{
				$sel_trackingno		=	"SELECT TRACKINGNO from DISPATCH_TEST.DISPATCH_METROMANILA_DTL WHERE SOFNO = '{$OrderNo}' ";
			}
			else 
			{
				$sel_trackingno		=	"SELECT TRACKINGNO from DISPATCH_TEST.DISPATCH_PROVINCE_DTL WHERE SOFNO = '{$OrderNo}' ";
			}
			//echo $sel_trackingno;
			...
			if ($rssel_trackingno->RecordCount() > 0)
			{
				$TRACKINGNO	=	$rssel_trackingno->fields['TRACKINGNO'];
				
				if ($OPT2=='MANILA') 
				{
					$sel_dispatch	=	"SELECT STATUS FROM DISPATCH_TEST.DISPATCH_METROMANILA_HDR WHERE TRACKINGNO = '{$TRACKINGNO}' ";
				}
				else 
				{
					$sel_dispatch	=	"SELECT STATUS FROM DISPATCH_TEST.DISPATCH_PROVINCE_HDR WHERE TRACKINGNO = '{$TRACKINGNO}' ";
				}
				...
				if ($STATUS=='PRINTED') 
				{
					...
					$sel_D		=	"SELECT sum(QTY)as RCVD_QTY from WMS_NEW.RECEIVINGFORM_DTL where SOF = '{$OrderNo}' ";
					...
					$sel_D		=	"SELECT  SKUNO,QTY,UNITCOST,UNITPRICE,GROSSAMT,NETAMT from WMS_NEW.RECEIVINGFORM_DTL where SOF = '{$OrderNo}' ";
					$rssel_D	=	$Filstar_conn->Execute($sel_D);
					if ($rssel_D==false) 
					{
						echo $Filstar_conn->ErrorMsg()."::".__LINE__;exit();
						...
						{
							$CNT++;
							$Item		=	$rssel_D->fields['SKUNO'];
							$ItemDesc	=	substr($global_func->Select_val($Filstar_conn,"FDCRMSlive","itemmaster","ItemDesc","ItemNo='{$Item}'"),0,35);
							
							$OrderQty	=	$rssel_D->fields['QTY'];
							$ReleaseQty	=	$rssel_D->fields['QTY'];
							$UnitCost	=	$rssel_D->fields['UNITCOST'];
							$UnitPrice	=	$rssel_D->fields['UNITPRICE'];
							$GrossAmount=	number_format($rssel_D->fields['GROSSAMT'],2);
							$NetAmount	=	number_format($rssel_D->fields['NETAMT'],2);
							
							$total_order+=	$OrderQty;
							$total_rcvd	+=	$ReleaseQty;
							$total_gross+=	$rssel_D->fields['GROSSAMT'];
							$total_net	+=	$rssel_D->fields['NETAMT'];
							...
							$show	.=	"<input type='hidden' name='new_net_$CNT' id='new_net_$CNT' value='{$rssel_D->fields['NETAMT']}' size='5'>";
							$show	.=	"<input type='hidden' name='old_net_$CNT' id='old_net_$CNT' value='{$rssel_D->fields['NETAMT']}' size='5'>";
							$show	.=	"</td>";
							
							$show	.=	"<td align='center'>";
							$show	.=	$UnitPrice;
							$show	.=	"<input type='hidden' name='txtUnitprice_$CNT' id='txtUnitprice_$CNT' value='{$UnitPrice}' size='5'>";
							$show	.=	"<input type='hidden' name='new_gross_$CNT' id='new_gross_$CNT' value='{$rssel_D->fields['GROSSAMT']}' size='5'>";
							$show	.=	"<input type='hidden' name='old_gross_$CNT' id='old_gross_$CNT' value='{$rssel_D->fields['GROSSAMT']}' size='5'>";
							$show	.=	"</td>";
	...
	if ($action=='CONFIRM__') 
	{
		...
		$sel_data	=	"SELECT SOF,CUSTCODE,RFNO,GROSSAMOUNT,NETAMOUNT from WMS_NEW.RECEIVINGFORM_HDR where RFNO = '{$VAL__}' ";
		$rssel_data	=	$Filstar_conn->Execute($sel_data);
		if ($rssel_data==false) 
		{
			echo $Filstar_conn->ErrorMsg()."::".__LINE__;exit();
		...
		$OrderNo		=	$rssel_data->fields['SOF'];
		$CustNo			=	$rssel_data->fields['CUSTCODE'];
		$InvoiceNo		=	$rssel_data->fields['RFNO'];
		$OrderAmount	=	$rssel_data->fields['GROSSAMOUNT'];
		$InvoiceAmount	=	$rssel_data->fields['NETAMOUNT'];
		...
		for ($y=1;$y<=$record;$y++)
		{
			$item			=	$_POST['hdn_items_'.$y];
			$del_qty		=	$_POST['txtDelQty_'.$y];
			$rvc_qty		=	$_POST['txtRelQty_'.$y];
			$gross_new		=	$_POST['new_gross_'.$y];
			$gross_old		=	$_POST['old_gross_'.$y];
			$net_new		=	$_POST['new_net_'.$y];
			$net_old		=	$_POST['old_net_'.$y];
			$remarks_		=	$_POST['remarks_'.$y];
			$hdn_remarks_	=	$_POST['hdn_remarks_'.$y];
			...
			if ($del_qty!=$rvc_qty) 
			{
				if (!empty($item)) 
				{
					...
					$insert_IA	=	"INSERT INTO WMS_NEW.INVENTORYADJUSTMENT_DTL(`IATRANSN0`,`SKUNO`,`IAQTY`,`REMARKS`) 
									VALUES
		...
		<table width="100%" border="0" class="Text_header" style="border:0px">
			<tr>
				<td align="right" width="42%">
					<input type="radio" name="rdtype1" id="rdMANILA" value="MANILA" checked onclick="CLEAR_TXTBOX();">
				</td>
				<td align="center" width="5%" style="font-size:20px">
					MANILA
				</td>
				<td align="center" width="5%">
					<input type="radio" name="rdtype1" id="rdPROVINCIAL" value="PROVINCE"  onclick="CLEAR_TXTBOX();">
				</td>
		...
		<table width="100%" border="0" class="Text_header" style="border:0px">
			<tr>
				...
				<td align="left" width="55%" colspan="3" style="font-size:20px">
					...
					<input type="text" name="notload" id="notload" value="" style="font-size:20px" readonly>
				</td>
			</tr>
		</table>
		<!--</div>-->
