...
if(!empty($_POST['user']) && !empty($_POST['pass']))
{
	...
	$cnt 	=	"SELECT COUNT(*) AS CNT FROM WMS_USERS.USERS WHERE USERNAME = '{$user}' AND PASSWORD = '{$pass}' AND STATUS = 'Active' ";
	
	$rscnt	=	$Filstar_conn->Execute($cnt);
	if ($rscnt->fields['CNT'] > 0) 
	{
		$sel	=	"SELECT * FROM WMS_USERS.USERS WHERE USERNAME = '{$user}' AND PASSWORD = '{$pass}'  AND STATUS = 'Active'";
		$rsel	=	$Filstar_conn->Execute($sel);
		while (!$rsel->EOF) 
		{
			$_SESSION['NAME']			=	$rsel->fields['NAME'];
