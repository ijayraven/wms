...
if(!empty($_POST['user']) && !empty($_POST['pass']))
{
	...
	$cnt 	=	"SELECT COUNT(*) AS CNT FROM ".WMS_LOOKUP.".USER WHERE USERNAME = '{$user}' AND PASSWORD = '{$pass}' AND STATUS = 'Y' ";
	
	$rscnt	=	$Filstar_conn->Execute($cnt);
	if ($rscnt->fields['CNT'] > 0) 
	{
		$sel	=	"SELECT * FROM ".WMS_LOOKUP.".USER WHERE USERNAME = '{$user}' AND PASSWORD = '{$pass}'  AND STATUS = 'Y'";
