...
class	__Global_Func
{
	...
	/**
	 * AdminErrorlogs Create error logs
	 *
	 * @param SQL $queryko
	 * @param ERROR $errorko
	 * @param __FILE__ $filename
	 * @param __LINE__ $linenum
	 */
	public function AdminErrorLogs($queryko="",$errorko,$filename,$linenum)
	{
		$dt = date("Y-m-d");
		$errdt = date("Y-m-d H:i:s");
		exec("/sbin/ifconfig", $ips);
		$tmp = explode(" ", $ips[1]);
		$tmp = str_replace("addr:", "", $tmp[11]);		
		$cIP = $tmp;
		
		// save to the error log, and e-mail me if there is a critical user error
		$errLog = "========================================\n";
	    $errLog .= "[DATETIME]"		.$errdt . 			"\n";
	    $errLog .= "[SERVERADDR]"		.$cIP . 		"\n";
	    $errLog .= "[QUERYHERE]"		.$queryko . 	"\n";
	    $errLog .= "[ERRORMSG]"		.$errorko . 	"\n";
	    $errLog .= "[SCRIPTNAME]"		.$filename . 	"\n";
	    $errLog .= "[SCRIPTLINENUM]"		.$linenum . 	"\n\n";
	    
	    if (is_dir(ERRLOG_PATH)) 
	    {
		    chmod(ERRLOG_PATH,0777);
	    }
	    else
	    {
	    	mkdir(ERRLOG_PATH);
	    ...
	    $errorfile = ERRLOG_PATH.$dt."_".ERRFILENAME;
	    
	    error_log($errLog, 3, $errorfile);
	...
	/**
	 * AdminQuerlog Create query Logs
	 *
	 * @param SQL $queryko
	 * @param __FILE__  $filename
	 * @param __LINE__  $linenum
	 */
	public function AdminQueryLogs($queryko="",$filename,$linenum)
	{
	    ...
	    $qLog .= "[DATETIME]"		.$errdt . 			"\n";
	    $qLog .= "[SERVERADDR]"		.$cIP . 		"\n";
	    $qLog .= "[QUERYHERE]"		.$queryko . 	"\n";
	    $qLog .= "[SCRIPTNAME]"		.$filename . 	"\n";
	    $qLog .= "[SCRIPTLINENUM]"		.$linenum . 	"\n\n";
	    
	    if (is_dir(QUERY_PATH)) 
	    {
		    chmod(QUERY_PATH,0777);
	    }
	    else
	    {
	    	mkdir(QUERY_PATH);
	    ...
	    $qfile = QUERY_PATH.$dt."_".QFILENAME;
	    
	    error_log($qLog, 3, $qfile);
	    if (is_file($qfile)) 
	    {
	    	chmod($qfile,0777);
	    }	
	 ...
	 * @return string
	 */
	public function	Login_user(&$conn,$val_user)
	{
		try {
			$conn->StartTrans();
			...
			if ($rssel == false) 
			{
				self::AdminErrorLogs($conn->ErrorMsg(),__FILE__,__LINE__);
	 ...
	 * @return String
	 */
	public function	CustName($conn,$val_no)
	{
		try {
			$conn->StartTrans();
			...
			if ($rsdept == false) 
			{
				self::AdminErrorLogs($sel_dept,$conn->ErrorMsg(),__FILE__,__LINE__);
				throw new Exception($conn->ErrorMsg());
			}
	 ...
	 * @return String
	 */
	public function	Generate_transeq($conn,$type)
	{
		$date	=	date('Ymd');
		try {
			$conn->StartTrans();
			if ($type == 'MANILA') 
			{
				...
				if ($rstranseq == false) 
				{
					self::AdminErrorLogs($transeq,$conn->ErrorMsg(),__FILE__,__LINE__);
					throw new Exception($conn->ErrorMsg());
				}
				$id	=	$conn->Insert_ID();
	 ...
	 * @return unknown
	 */
	public function	Select_val($conn,$db,$tbl,$fld,$condition)
	{
		try {
			$conn->StartTrans();
	 ...
	 * @param unknown_type $conn2
	 * @param unknown_type $data
	 * @param unknown_type $type
	 */
	public function	Table_form($conn1,$conn2,$data,$type)
	{
		$cnt	=	1;
		if ($type	==	'MANILA') 
		{
			...
			$show	.=	"<table width='100%' border='0' class='d0'>";
				...
				foreach ($data as $key=>$val)
				{
					...
					for ($x;$x<=($val_cnt - 1);$x++)
					{
						...
						$Street			=	self::Select_val($conn2,FDCRMS,"customer_address","StreetNumber","custno= '$key'");
						$Town			=	self::Select_val($conn2,FDCRMS,"customer_address","TownCity","custno= '$key'");
						$Custname_addr	=	self::CustName($conn2,$key)."<br>".$Street.' '.$Town;
...
//			/******************************************************************/
			$show	.=	"<table width='100%' border='0' class='d0'>";
				...
				foreach ($data as $key=>$val)
				{
					...
					for ($x;$x<=($val_cnt - 1);$x++)
					{
						...
						$invoiceamt		=	self::Select_val($conn1,DISPATCH_DB,"DISPATCH_ORDER","INVOICEAMOUNT","CUSTNO= '$key' AND INVOICENO= '".$val[$x]."'");
						$orderno		=	self::Select_val($conn1,DISPATCH_DB,"DISPATCH_ORDER","ORDERNO","CUSTNO= '$key' AND INVOICENO= '".$val[$x]."'");
						$picklist		=	self::Select_val($conn2,FDCRMS,"orderheader","PickListNo","CustNo= '$key' AND InvoiceNo= '".$val[$x]."'");
		...
		elseif ($type == 'PROVINCE')
		{
			...
			$show	.=	"<table width='100%' border='0' class='d0'>";
										...
										$show	.=	"</td>";
												...
												$show	.=	"</td>";
														...
														$show	.=	"</td>";
																...
																$show	.=	"DRIVER";
																$show	.=	"</td>";
																		$show	.=	"<td width='39%' align='left' class='Text_header' colspan='5'>";
					...
					$show	.=	"<tr>";
						...
						$show	.=	"</td>";
								...
								$show	.=	"</td>";
										...
										$show	.=	"</td>";
												$show	.=	"<td width='5%' align='center' class='Text_header'>";
												$show	.=	"<input type='text' name='txtvan_PROVINCE' id='txtvan_PROVINCE' value='' size='3'  onkeyup=search_van(event,this.value,this.id,'$type'); style='text-align:center;'>";
												$show	.=	"<div id='div_van_PROVINCE' style='position:absolute;'></div>";
												$show	.=	"</td>";
														$show	.=	"<td width='5%' align='center' class='Text_header'>";
														$show	.=	"<input type='text' name='txtplate_PROVINCE' id='txtplate_PROVINCE' value='' size='3' style='text-align:center;' readonly>";
														$show	.=	"</td>";
																$show	.=	"<td width='10%' align='left' class='Text_header'>";
																$show	.=	"HELPER";
																$show	.=	"</td>";
					...
					$show	.=	"</tr>";
					$show	.=	"<tr>";
						$show	.=	"<td width='61%' align='left' colspan='5' class='Text_header'>";
						$show	.=	"&nbsp";
						$show	.=	"</td>";
							$show	.=	"<td width='10%' align='left' class='Text_header'>";
							$show	.=	"FORWARDER";
									...
									$show	.=	"</td>";
											...
											$show	.=	"</td>";
													...
													$show	.=	"</td>";
															...
															$show	.=	"QTY";
															$show	.=	"</td>";
																	$show	.=	"<td width='7%'  align='center' rowspan='2' class='Text_header'>";
																	$show	.=	"DR NO.";
																	$show	.=	"</td>";
																			$show	.=	"<td width='6%'  align='center' class='Text_header'>";
																			$show	.=	"QTY";
																			$show	.=	"</td>";
																				$show	.=	"<td width='6%'  align='center' class='Text_header'>";
																				$show	.=	"QTY";
														...
														$show	.=	"</td>";
															...
															$show	.=	"</td>";
																	...
																	$show	.=	"</td>";
																			...
																			$show	.=	"NUMBER";
																			$show	.=	"</td>";
																						$show	.=	"<td width='7%'  align='center' nowrap class='Text_header'>";
																						$show	.=	"BY KILO";
																						$show	.=	"</td>";
								...
								$show	.=	"</td>";
										...
										$show	.=	"</td>";
												...
												$show	.=	"</td>";
														...
														$show	.=	"<input type='text' name='txt_PROVINCE_size_$cnt' id='txt_PROVINCE_size_$cnt' value='' size='3' style='text-align:center;border:1;' maxlength='4' onkeyup='isnumeric(this.value,this.id);'>";
														$show	.=	"</td>";
																$show	.=	"<td width='10%' align='center' onclick=focus_here('txt_PROVINCE_ctn_$cnt'); class='Text_header'>";
																$show	.=	"<input type='text' name='txt_PROVINCE_ctn_$cnt' id='txt_PROVINCE_ctn_$cnt' value='' size='1' style='text-align:center;border:1;' maxlength='4' onkeyup='isnumeric(this.value,this.id);'>";
																$show	.=	"</td>";
																		$show	.=	"<td width='7%'  align='center'  onclick=focus_here('txt_PROVINCE_dr_$cnt'); class='Text_header'>";
																		$show	.=	"<input type='text' name='txt_PROVINCE_dr_$cnt' id='txt_PROVINCE_dr_$cnt' value='' size='1' style='text-align:center;border:1;' maxlength='7'>";
																		$show	.=	"</td>";
																				$show	.=	"<td width='6%'  align='center' onclick=focus_here('txt_PROVINCE_pkg_$cnt'); class='Text_header'>";
																				$show	.=	"<input type='text' name='txt_PROVINCE_pkg_$cnt' id='txt_PROVINCE_pkg_$cnt' value='' size='1' style='text-align:center;border:1;' maxlength='4' onkeyup='isnumeric(this.value,this.id);'>";
																				$show	.=	"</td>";
																						$show	.=	"<td width='6%'  align='center' onclick=focus_here('txt_PROVINCE_bdl_$cnt'); class='Text_header'>";
																						$show	.=	"<input type='text' name='txt_PROVINCE_bdl_$cnt' id='txt_PROVINCE_bdl_$cnt' value='' size='1' style='text-align:center;border:1;' maxlength='4' onkeyup='isnumeric(this.value,this.id);'>";
																						$show	.=	"</td>";
																									$show	.=	"<td width='10%'  align='center' onclick=focus_here('txt_PROVINCE_bill_$cnt');  class='Text_header'>";
																									$show	.=	"<input type='text' name='txt_PROVINCE_bill_$cnt' id='txt_PROVINCE_bill_$cnt' value='' size='1' style='text-align:center;border:1;' maxlength='10'>";
																									$show	.=	"</td>";
																											$show	.=	"<td width='7%'  align='center' onclick=focus_here('txt_PROVINCE_kilo_$cnt'); class='Text_header'>";
																											$show	.=	"<input type='text' name='txt_PROVINCE_kilo_$cnt' id='txt_PROVINCE_kilo_$cnt' value='' size='1' style='text-align:center;border:1;' maxlength='4'>";
																											$show	.=	"</td>";
						$show	.=	"</tr>";
						$cnt++;
						}
			...
			$show	.=				"SPECIAL INSTRUCTION";
			$show	.=			"</td>";
			$show	.=			"<td width='75%' align='left' class='Text_header' colspan='10'>";
...
//		unset($data);
	 ...
	 * @param connect 2 $conn2
	 * @param dispatch type $type
	 * @param tracking no. $transeq
	 */
	public function	Table_form_update($conn1,$conn2,$type,$transeq)
	{
		$cnt	=	1;
		$aData	=	array();
		if ($type	==	'MANILA') 
			...
			$show	.=	"<table width='100%' border='0' class='d0'>";
				...
				foreach ($aData as $transeq=>$val_custcode)
				{
					foreach ($val_custcode as $custcode=>$val_invoice) 
					{
						...
						{
								$Street			=	self::Select_val($conn2,FDCRMS,"customer_address","StreetNumber","custno= '$custcode'");
								$Town			=	self::Select_val($conn2,FDCRMS,"customer_address","TownCity","custno= '$custcode'");
								$Custname_addr	=	self::CustName($conn2,$custcode)."<br>".$Street.' '.$Town;
...
//			/******************************************************************/
		...
		elseif ($type == 'PROVINCE')
		{
			...
			$sel	=" SELECT `ID_DTL`,`TRANSEQ`,`CUSTCODE`, `INVOICENO`, `INVOICEAMOUNT`, `SOFNO`, `PRODUCT_LINE`, `SIZE_OF_CTN`, `QTY_CTN`, `DR_NO`,`QTY_PKG`,`QTY_BDL`,`WAYBILL_NUMBER`, `WEIGHT_BY_KILO` ";
					...
					$show	.=	"<tr>";
						...
						$show	.=	"</td>";
								...
								$show	.=	"</td>";
										...
										$show	.=	"</td>";
												$show	.=	"<td width='5%' align='center' class='Text_header'>";
												$show	.=	"<input type='text' name='txtvan_PROVINCE' id='txtvan_PROVINCE' value='$vanno' size='3'  onkeyup=search_van(event,this.value,this.id,'$type'); style='text-align:center;'>";
												$show	.=	"<div id='div_van_PROVINCE' style='position:absolute;'></div>";
												$show	.=	"</td>";
														$show	.=	"<td width='5%' align='center' class='Text_header'>";
														$show	.=	"<input type='text' name='txtplate_PROVINCE' id='txtplate_PROVINCE' value='$plate' size='3' style='text-align:center;' readonly>";
														$show	.=	"</td>";
																$show	.=	"<td width='10%' align='left' class='Text_header'>";
																$show	.=	"HELPER";
																$show	.=	"</td>";
									...
									$show	.=	"</td>";
										...
										$show	.=	"</td>";
												...
												$show	.=	"</td>";
														...
														$show	.=	"</td>";
																$show	.=	"<td width='8%' align='center' onclick=focus_here('txt_PROVINCE_size_$cnt'); class='Text_header'>";
																$show	.=	"<input type='text' name='txt_PROVINCE_size_$cnt' id='txt_PROVINCE_size_$cnt' value='".$val['SIZE_OF_CTN']."' size='1' style='text-align:center;border:1;' maxlength='4' onkeyup='isnumeric(this.value,this.id);'>";
																$show	.=	"</td>";
																		$show	.=	"<td width='10%' align='center' onclick=focus_here('txt_PROVINCE_ctn_$cnt'); class='Text_header'>";
																		$show	.=	"<input type='text' name='txt_PROVINCE_ctn_$cnt' id='txt_PROVINCE_ctn_$cnt' value='".$val['QTY_CTN']."' size='1' style='text-align:center;border:1;' maxlength='4' onkeyup='isnumeric(this.value,this.id);'>";
																		$show	.=	"</td>";
																				$show	.=	"<td width='7%'  align='center'  onclick=focus_here('txt_PROVINCE_dr_$cnt'); class='Text_header'>";
																				$show	.=	"<input type='text' name='txt_PROVINCE_dr_$cnt' id='txt_PROVINCE_dr_$cnt' value='".$val['DR_NO']."' size='1' style='text-align:center;border:1;' maxlength='7'>";
																				$show	.=	"</td>";
																						$show	.=	"<td width='6%'  align='center' onclick=focus_here('txt_PROVINCE_pkg_$cnt'); class='Text_header'>";
																						$show	.=	"<input type='text' name='txt_PROVINCE_pkg_$cnt' id='txt_PROVINCE_pkg_$cnt' value='".$val['QTY_PKG']."' size='1' style='text-align:center;border:1;' maxlength='4' onkeyup='isnumeric(this.value,this.id);'>";
																						$show	.=	"</td>";
																								$show	.=	"<td width='6%'  align='center' onclick=focus_here('txt_PROVINCE_bdl_$cnt'); class='Text_header'>";
																								$show	.=	"<input type='text' name='txt_PROVINCE_bdl_$cnt' id='txt_PROVINCE_bdl_$cnt' value='".$val['QTY_BDL']."' size='1' style='text-align:center;border:1;' maxlength='4' onkeyup='isnumeric(this.value,this.id);'>";
																								$show	.=	"</td>";
																										$show	.=	"<td width='10%'  align='center' onclick=focus_here('txt_PROVINCE_bill_$cnt');  class='Text_header'>";
																										$show	.=	"<input type='text' name='txt_PROVINCE_bill_$cnt' id='txt_PROVINCE_bill_$cnt' value='".$val['WAYBILL_NUMBER']."' size='1' style='text-align:center;border:1;' maxlength='10'>";
																										$show	.=	"</td>";
																												$show	.=	"<td width='7%'  align='center' onclick=focus_here('txt_PROVINCE_kilo_$cnt'); class='Text_header'>";
																												$show	.=	"<input type='text' name='txt_PROVINCE_kilo_$cnt' id='txt_PROVINCE_kilo_$cnt' value='".$val['WEIGHT_BY_KILO']."' size='1' style='text-align:center;border:1;' maxlength='4'>";
																												$show	.=	"</td>";
								$show	.=	"</tr>";
								$cnt++;
							}
...
//		unset($data);
		 ...
		 * @return string
		 */
		public function str_concat($string,$length=25)
		{
			$count = 0;
			$newstring = array();
		...
		function Remove_alpha($val)
		{
			$v	=	substr($val,-1);
			if (is_numeric($v))
			{
				$prodline	=	substr($val,1,1);
			}
			else 
			{
				$prodline	=	substr($val,-1);
			}
			return $prodline;
		}
